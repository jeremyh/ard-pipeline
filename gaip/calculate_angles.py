    ...
    if lon_fname == lat_fname:
        with h5py.File(lon_fname, "r") as src:
            ...
            fid = calculate_angles(
                acquisition, lon_ds, lat_ds, npoints, out_fname, compression
            )
    else:
...
def create_boxline_coordinator(
    ...
    coordinator_dtype = np.dtype([("row_index", "int64"), ("col_index", "int64")])
...
def calculate_angles(
    ...
    if out_fname is None:
        fid = h5py.File("satellite-solar-angles", driver="core", backing_store=False)
    else:
    ...
    desc = (
        "The satellite acquisition time grid in seconds before and after "
    ...
    centreline_dataset = create_centreline_dataset(geobox, y_cent, x_cent, n_cent)
    ...
    attrs = {"Description": desc, "array_coordinate_offset": -1}
