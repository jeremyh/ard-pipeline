...
def _self_shadow(
    ...
    with h5py.File(incident_angles_fname, "r") as inci_angles, h5py.File(
        ...
        fid = self_shadow(inci_dset, exit_dset, geobox, out_fname, compression, y_tile)
...
def self_shadow(
    incident_dataset, exiting_dataset, geobox, out_fname, compression="lzf", y_tile=None
):
...
def _combine_shadow(
    ...
    with h5py.File(self_shadow_fname, "r") as fid_self, h5py.File(
        ...
        fid = combine_shadow_masks(
            self_shadow, cast_sun, cast_sat, geobox, out_fname, compression, y_tile
        )
...
def combine_shadow_masks(
    ...
    for tile in tiles:
        ...
        out_dset[idx] = (
            self_shadow[idx] & cast_shadow_sun[idx] & cast_shadow_satellite[idx]
        )
