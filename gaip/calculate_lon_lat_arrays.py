...
def create_lon_lat_grids(
    acquisition, depth=7, dtype="float64", out_fname=None, compression="lzf"
):
...
def create_lon_grid(
    acquisition, out_fname=None, compression="lzf", depth=7, dtype="float64"
):
    ...
    # Initialise the output files
    if out_fname is None:
        fid = h5py.File("longitude.h5", driver="core", backing_store=False)
    else:
        fid = h5py.File(out_fname, "w")
...
def create_lat_grid(
    ...
    # Initialise the output files
    if out_fname is None:
        fid = h5py.File("latitude.h5", driver="core", backing_store=False)
    else:
        fid = h5py.File(out_fname, "w")
