...
from gaip import interpolate_grid, write_img
...
def interpolate_array(shape, eval_func, depth=7, dtype="float64"):
    ...
    ndims = len(shape)
    if len(ndims) != 2:
        err = f"Dimensions must be 2D; Received: {ndims}"
    ...
    interpolate_grid(
        depth=depth, origin=(0, 0), shape=shape, eval_func=eval_func, grid=array
    )
...
def create_lon_lat_grids(
    geobox, lon_fname="LON.tif", lat_fname="LAT.tif", work_dir="", to_disk=True
):
