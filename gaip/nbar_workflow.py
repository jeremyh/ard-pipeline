...
class GetAncillaryData(luigi.Task):
    ...
    def output(self):
        out_path = acquisitions(self.level1).get_root(self.work_root, self.granule)
...
class RunModtranCase(luigi.Task):
    ...
    def output(self):
        ...
        out_fname = "".join(
            [POINT_ALBEDO_FMT.format(p=self.point, a=self.albedo), ".h5"]
        )
...
class AccumulateSolarIrradiance(luigi.Task):
    ...
    def run(self):
        ...
        with self.output().temporary_path() as out_fname:
            _calculate_solar_radiation(acq, self.input(), out_fname, self.compression)
...
class TerrainCorrection(luigi.Task):
    ...
    def requires(self):
        ...
        for band in bands_to_process:
            reqs.append(
                RunTCBand(
                    self.level1, self.work_root, self.granule, self.group, band_num=band
                )
            )
        return reqs
