...
def _run_modtran(modtran_exe, workpath, point, albedo, out_fname, compression="lzf"):
    """A private wrapper for dealing with the internal custom workings of the
    NBAR workflow.
    """
    fid = run_modtran(modtran_exe, workpath, point, albedo, out_fname, compression)
...
def run_modtran(
    modtran_exe, workpath, point, albedo, out_fname=None, compression="lzf"
):
    ...
    pd.read_csv(chn_fname, skiprows=5, header=None, delim_whitespace=True)
...
def _calculate_coefficients(
    accumulated_fname, channel_fname, npoints, out_fname, compression="lzf"
):
    """A private wrapper for dealing with the internal custom workings of the
    NBAR workflow.
    """
    with h5py.File(accumulated_fname, "r") as fid1, h5py.File(
        channel_fname, "r"
    ) as fid2:
        fid = calculate_coefficients(fid1, fid2, npoints, out_fname, compression)
...
def calculate_coefficients(
    accumulation_fid, channel_fid, npoints, out_fname=None, compression="lzf"
):
    ...
    write_dataframe(result, "coefficients-format-1", fid, compression, attrs=attrs1)
    write_dataframe(result2, "coefficients-format-2", fid, compression, attrs=attrs2)
    ...
    with h5py.File(out_fname, "w") as fid:
        for key in flux_fnames:
            ...
            with h5py.File(flux_fnames[key], "r") as fid2:
                flux_data = pd.DataFrame(fid2[flux_dset_name][:])
                flux_data.set_index(
                    ["wavelength", flux_data.index], drop=False, inplace=True
                )
                levels = fid2[atmos_dset_name].attrs["altitude levels"]

            # accumulate solar irradiance
            df = calculate_solar_radiation(
                flux_data, response_fname, levels, transmittance
            )
...
def calculate_solar_radiation(
    flux_data, response_fname, levels=36, transmittance=False
):
