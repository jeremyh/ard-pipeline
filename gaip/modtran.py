...
def _run_modtran(modtran_exe, workpath, point, albedo, out_fname, compression="lzf"):
    ...
    NBAR workflow.
    """
    rfid = run_modtran(modtran_exe, workpath, point, albedo, out_fname, compression)
...
def calculate_coefficients(
    ...
    for point in points:
        ...
        factors = ["fs", "fv", "a", "b", "s", "dir", "dif", "ts"]
    ...
    for bn, grp in groups:
        for factor in factors:
            ...
            result2[(bn, factor)] = pd.DataFrame(
...
def _calculate_solar_radiation(acquisition, flux_fnames, out_fname, compression="lzf"):
    """A private wrapper for dealing with the internal custom workings of the
    NBAR workflow.
    """
