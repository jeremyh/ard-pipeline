...
def run_tc(
    ...
    self_shadow = as_array(
        read_image(self_shadow_fname), dtype=numpy.int16, transpose=True
    )
    cast_shadow_sun = as_array(
        read_image(cast_shadow_sun_fname), dtype=numpy.int16, transpose=True
    )
    cast_shadow_satellite = as_array(
        read_image(cast_shadow_satellite_fname), dtype=numpy.int16, transpose=True
    )
    solar_zenith = as_array(
        read_image(solar_zenith_fname), dtype=numpy.float32, transpose=True
    )
    solar_azimuth = as_array(
        read_image(solar_azimuth_fname), dtype=numpy.float32, transpose=True
    )
    satellite_view = as_array(
        read_image(satellite_view_fname), dtype=numpy.float32, transpose=True
    )
    relative_angle = as_array(
        read_image(relative_angle_fname), dtype=numpy.float32, transpose=True
    )
    slope = as_array(read_image(slope_fname), dtype=numpy.float32, transpose=True)
    aspect = as_array(read_image(aspect_fname), dtype=numpy.float32, transpose=True)
    incident_angle = as_array(
        read_image(incident_angle_fname), dtype=numpy.float32, transpose=True
    )
    exiting_angle = as_array(
        read_image(exiting_angle_fname), dtype=numpy.float32, transpose=True
    )
    relative_slope = as_array(
        read_image(relative_slope_fname), dtype=numpy.float32, transpose=True
    )
