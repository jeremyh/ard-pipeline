...
def incident_angles(
    ...
    no_data = -999
    outds_incident = tiling.TiledOutput(
        incident_out_fname, cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    outds_azi_incident = tiling.TiledOutput(
        azimuth_incident_out_fname,
        cols,
        rows,
        geobox=geobox,
    ...
    with rasterio.open(solar_zenith_fname) as sol_z_ds, rasterio.open(
        ...
        for tile in tiles:
            ...
            sol_zen = as_array(
                sol_z_ds.read_band(1, window=tile, masked=False),
                dtype=np.float32,
                transpose=True,
            )
            sol_azi = as_array(
                sol_azi_ds.read_band(1, window=tile, masked=False),
                dtype=np.float32,
                transpose=True,
            )
            slope = as_array(
...
def exiting_angles(
    ...
    out_dtype = gdal.GDT_Float32
    no_data = -999
    outds_exiting = tiling.TiledOutput(
        exiting_out_fname, cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    outds_azi_exiting = tiling.TiledOutput(
        azimuth_exiting_out_fname,
        cols,
        rows,
        geobox=geobox,
    ...
    with rasterio.open(satellite_view_fname) as sat_view_ds, rasterio.open(
        ...
        for tile in tiles:
            ...
            # Convert to required datatype and transpose
            sat_view = as_array(
                sat_view_ds.read_band(1, window=tile, masked=False),
                dtype=np.float32,
                transpose=True,
            )
            sat_azi = as_array(
                sat_azi_ds.read_band(1, window=tile, masked=False),
                dtype=np.float32,
                transpose=True,
            )
            slope = as_array(
