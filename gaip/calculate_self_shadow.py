...
def self_shadow(
    incident_fname, exiting_fname, self_shadow_out_fname, x_tile=None, y_tile=None
):
    ...
    with rasterio.open(incident_fname) as inc_ds, rasterio.open(
        ...
        outds = tiling.TiledOutput(self_shadow_out_fname, cols, rows, geobox=geobox)
        ...
        for tile in tiles:
            ...
            inc = np.radians(inc_ds.read(1, window=tile, masked=False))
            exi = np.radians(exi_ds.read(1, window=tile, masked=False))
