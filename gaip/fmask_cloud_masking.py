...
def nd2toarbt(filename, images=None):
    ...
    if Lnum >= 4 and Lnum <= 7:
        ...
        im_B2 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B2 - Qmi) + Rmi",
            {"Rma": Refmax[0], "Rmi": Refmin[0], "Qma": Qcalmax[0], "Qmi": Qcalmin[0]},
            locals(),
        )
        im_B3 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B3 - Qmi) + Rmi",
            {"Rma": Refmax[1], "Rmi": Refmin[1], "Qma": Qcalmax[1], "Qmi": Qcalmin[1]},
            locals(),
        )
        im_B4 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B4 - Qmi) + Rmi",
            {"Rma": Refmax[2], "Rmi": Refmin[2], "Qma": Qcalmax[2], "Qmi": Qcalmin[2]},
            locals(),
        )
        im_B5 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B5 - Qmi) + Rmi",
            {"Rma": Refmax[3], "Rmi": Refmin[3], "Qma": Qcalmax[3], "Qmi": Qcalmin[3]},
            locals(),
        )
        im_B6 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B6 - Qmi) + Rmi",
            {"Rma": Refmax[4], "Rmi": Refmin[4], "Qma": Qcalmax[4], "Qmi": Qcalmin[4]},
            locals(),
        )
        im_B7 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B7 - Qmi) + Rmi",
            {"Rma": Refmax[5], "Rmi": Refmin[5], "Qma": Qcalmax[5], "Qmi": Qcalmin[5]},
            locals(),
        )
        im_B9 = numexpr.evaluate(
            "((Rma - Rmi) / (Qma - Qmi)) * (im_B9 - Qmi) + Rmi",
            {"Rma": Refmax[6], "Rmi": Refmin[6], "Qma": Qcalmax[6], "Qmi": Qcalmin[6]},
            locals(),
        )
        im_B10 = numexpr.evaluate(
            "((Lma - Lmi) / (Qma - Qmi)) * (im_B10 - Qmi) + Lmi",
            {"Lma": Lmax[7], "Lmi": Lmin[7], "Qma": Qcalmax[7], "Qmi": Qcalmin[7]},
            locals(),
        )
...
def plcloud(
    ...
    Snow[
        numexpr.evaluate(
            "(NDSI > 0.15) & (Temp < 1000) & (data4 > 1100) & (data2 > 1000)"
        )
    ] = 1
    # Snow[mask == 0] = 255
    # Water test
    ...
    WT[
        numexpr.evaluate(
            "((NDVI < 0.01) & (data4 < 1100)) | ((NDVI < 0.1) & (NDVI > 0) & (data4 < 500))"
        )
    ] = 1
    WT[mask == 0] = 255
    # ################################################ Whiteness test
    ...
    if ptm <= 0.1:  # no thermal test => meanless for snow detection (0~1)
        ...
        wclr_max = (
            scipy.stats.scoreatpercentile(wfinal_prob[idwt], 100 * h_pt) + cldprob
        ...
        final_prob = (
            100 * Temp_prob * Vari_prob + 100 * Thin_prob
        ...
        for cloud_type in s:
            ...
            for base_h in np.arange(Min_cl_height, Max_cl_height, i_step):
                ...
                tmp_xys[1, :], tmp_xys[0, :] = mat_truecloud(
                    orin_cid[1], orin_cid[0], h, A, B, C, omiga_par, omiga_per
                )  # Function is returned as (x_new,y_new)
...
if __name__ == "__main__":
    ...
    (
        ...
        geoT,
        prj,
    ) = plcloud(mtl, cldprob, num_Lst=Lnum, shadow_prob=True, log_filename=log_fname)
