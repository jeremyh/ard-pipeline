...
def fractional_cover(acquisitions, x_tile, y_tile, out_fnames):
    ...
    outds_pv = tiling.TiledOutput(
        out_fnames[0], cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    outds_npv = tiling.TiledOutput(
        out_fnames[1], cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    outds_bs = tiling.TiledOutput(
        out_fnames[2], cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    outds_ue = tiling.TiledOutput(
        out_fnames[3], cols, rows, geobox=geobox, dtype=out_dtype, nodata=no_data
    )
    ...
    for tile in tiles:
        ...
        (green, dead1, dead2, bare, err) = unmix(
            stack, sum_to_one_weight, endmembers_array
        )
