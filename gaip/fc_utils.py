...
def fractional_cover(acquisitions, x_tile, y_tile, out_fnames):
    ...
    if isinstance(acquisitions, LandsatAcquisition):
        ...
        bands = [
            acquisitions.bands.GREEN,
            acquisitions.bands.RED,
        ...
        for tile in tiles:
            ...
            # Read the data for the current tile from acquisitions
            img_d = get_dataset_data(acquisitions, bands, xs, ys, xsize, ysize)
            ...
            # Compute the fractions
            (grn, dead1, dead2, bare, err) = unmix(
                green, red, nir, swir1, swir2, sum_to_one_weight, endmembers_array
            )
