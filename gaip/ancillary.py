...
def get_aerosol_data(acquisition, aerosol_path, aot_loader_path=None):
    ...
    # be a coastal scene
    res = {"data_source": "default-value", "data_file": "default-value", "value": 0.06}
...
def get_solar_irrad(acquisitions, solar_path):
    ...
    with open(pjoin(solar_path, acqs[0].solar_irrad_file)) as infile:
        header = infile.readline()
        if "band solar irradiance" not in header:
            raise OSError("Cannot load solar irradiance file")
...
def get_solar_dist(acquisition, sundist_path):
    ...
    with open(sundist_path) as infile:
        for line in infile.readlines():
