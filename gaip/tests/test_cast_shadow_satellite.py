...
def calculate_cast_shadow_satellite(
    ...
    calculate_cast_shadow(
        ...
        block_height,
        block_width,
        fname_satellite_view,
        fname_satellte_azimuth,
        outfname,
    )
...
class TestCastShadowSatelliteFileNames(ParameterisedTestCase):
    ...
    def test_cast_shadow_satellite_tst(self):
        ...
        fname = pjoin(tc_dir, self.fname_view_shadow)
        msg = f"Test file does not exist: {fname}"
        assert pexists(fname) is True, msg
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Perform unittesting for the satellite view shadow mask and optionaly calculates the satellite view shadow mask."
    )
    ...
    parser.add_argument(
        "--compute",
        action="store_true",
        help="If set then the satellite view shadow mask will be computed before running the unittests.",
    )
    parser.add_argument(
        "--buffer",
        default=250,
        help="The buffer in pixels to be used in calculating the satellite view shadow.",
    )
    parser.add_argument(
    ...
    if compute:
        ...
        # Compute the mask
        calculate_cast_shadow_satellite(
            acqs[0], nbar_work_dir, outdir, buffer, block_y, block_x
        )
