...
class TestFilterFileNames(ParameterisedTestCase):
    ...
    def test_smoothed_dsm_tst(self):
        ...
        fname = pjoin(tc_dir, self.fname_smoothed_dsm)
        assert os.path.exists(fname) is True, "Test file does not exist: %s" % fname
...
if __name__ == "__main__":
    description = (
        "Performs unittests against the smoothed DSM and "
    ...
    parser.add_argument(
        "--compute",
        action="store_true",
        help=("If set then a smoothed dsm image will be " "created."),
    )
