...
class TestFCProducts(ParameterisedTestCaseFiles):
    ...
    def test_photosynthetic_veg_band(self):
        ...
        hist = h["histogram"]
        omin = h["omin"]
        cumu_h = np.cumsum(hist, dtype="float")
    ...
    def test_non_photosynthetic_veg_band(self):
        ...
        plt.plot(cdf)
        title = (
            "Cumulative Distribution of Differences for Non " "Photsynthetic Vegetation"
        )
        plt.title(title)
        plt.xlabel("Pixel Difference Value")
        plt.ylabel("Percent %")
...
"""
    ...
    md_template = template.format(
        pv_data=pv_data, npv_data=npv_data, bs_data=bs_data, ue_data=ue_data
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_filename", required=True, help="The filename for the test dataset."
    )
    parser.add_argument(
        "--pct_tolerance",
        default=3,
    ...
    suite.addTest(
        ParameterisedTestCaseFiles.parameterise(
            TestFCProducts,
