...
from gaip import acquisitions, find_file, read_img
from gaip.tests.unittesting_tools import ParameterisedTestCase

# TODO Filename to be determined from the nbar.cfg file


def calculate_cast_shadow_sun(
    acquisition, ref_dir, outdir, buffer=250, block_height=500, block_width=500
):
...
class TestCasthadowSunFileNames(ParameterisedTestCase):
    ...
    def test_cast_shadow_sun_ref(self):
        ...
        f"Reference file does not exist: {fname}"
        assert pexists(fname) is True, "Reference file does not exist: %s" % fname
    ...
    def test_cast_shadow_sun_tst(self):
        ...
        fname = pjoin(tc_dir, self.fname_cast_shadow_sun)
        f"Test file does not exist: {fname}"
        assert pexists(fname) is True, "Test file does not exist: %s" % fname
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Perform unittesting for the cast shadow sun mask and optionaly calculates the cast shadow sun mask."
    )
    ...
    parser.add_argument(
        "--compute",
        action="store_true",
        help="If set then the self shadow array will be computed before running the unittests.",
    )
    parser.add_argument(
        "--buffer",
        default=250,
        help="The buffer in pixels to be used in calculating the cast shadow sun mask.",
    )
    parser.add_argument(
        "--block_x", default=500, help="The x block size in pixels (Twice the buffer)."
    )
    parser.add_argument(
        "--block_y", default=500, help="The y block size in pixels (Twice the buffer).."
    )
    ...
    if compute:
        ...
        # Compute the angles
        calculate_self_shadow(acqs[0], nbar_work_dir, outdir, buffer, block_y, block_x)
