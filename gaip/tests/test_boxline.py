...
def read_coordinator(fname):
    df = pd.read_csv(
        fname, header=None, sep=r"\s+\s+", engine="python", names=["row", "col"]
    )
    return df
...
for d in dirs:
    ...
    print("boxline check")
    print("line check: {}".format((boxline_ref["line"] - boxline_test["line"]).sum()))
    print(
        "cstart check {}".format((boxline_ref["cstart"] - boxline_test["cstart"]).sum())
    )
    print("cend check: {}".format((boxline_ref["cend"] - boxline_test["cend"]).sum()))
    print("coordinator check")
    print(
        "row check: {}".format((coordinator_ref["row"] - coordinator_test["row"]).sum())
    )
    print(
        "col check: {}".format((coordinator_ref["col"] - coordinator_test["col"]).sum())
    )
