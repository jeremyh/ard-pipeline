...
class SlopeResultSet:
    ...
    def write_arrays(
        ...
        # Write
        write_img(
            mask_self, fname_mask_self, format=file_type, geobox=geobox, nodata=-999
        )
        write_img(slope, fname_slope, format=file_type, geobox=geobox, nodata=-999)
        write_img(aspect, fname_aspect, format=file_type, geobox=geobox, nodata=-999)
        write_img(
            incident, fname_incident, format=file_type, geobox=geobox, nodata=-999
        )
        write_img(exiting, fname_exiting, format=file_type, geobox=geobox, nodata=-999)
        write_img(
            azi_incident,
...
def run_brdfterrain(
    ...
    return terrain_correction(
        ...
        dd,
        ref_adj,
        as_array(dn_1, dtype=np.int16, transpose=True),
        as_array(mask_self, dtype=np.int16, transpose=True),
        as_array(mask_castsun, dtype=np.int16, transpose=True),
        as_array(mask_castview, dtype=np.int16, transpose=True),
        as_array(solar_angle, dtype=np.float32, transpose=True),
        as_array(sazi_angle, dtype=np.float32, transpose=True),
        as_array(view_angle, dtype=np.float32, transpose=True),
        as_array(rela_angle, dtype=np.float32, transpose=True),
        as_array(slope_angle, dtype=np.float32, transpose=True),
        as_array(aspect_angle, dtype=np.float32, transpose=True),
        as_array(it_angle, dtype=np.float32, transpose=True),
        as_array(et_angle, dtype=np.float32, transpose=True),
        as_array(rela_slope, dtype=np.float32, transpose=True),
        as_array(a_mod, dtype=np.float32, transpose=True),
        as_array(b_mod, dtype=np.float32, transpose=True),
        as_array(s_mod, dtype=np.float32, transpose=True),
        as_array(fs, dtype=np.float32, transpose=True),
        as_array(fv, dtype=np.float32, transpose=True),
        as_array(ts, dtype=np.float32, transpose=True),
        as_array(edir_h, dtype=np.float32, transpose=True),
        as_array(edif_h, dtype=np.float32, transpose=True),
    )
