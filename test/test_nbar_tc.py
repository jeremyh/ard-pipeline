...
class TestProductFileNames(ParameterisedTestCase):
    ...
    def test_lambert_files(self):
        ...
        assert len(ref_files) == len(test_files)
...
def test_compare_lmbrt_files(
    reference_directory, test_directory, output_directory, percent
):
    ...
    for i in range(len(ref_files)):
        ...
        diff_out_fname = pjoin(output_directory, fname.replace(".bin", "_diff.bin"))
        diff_fnames.append(diff_out_fname)
        write_img(diff, diff_out_fname, geobox=geobox)

        # Output the difference map
        out_fname = pjoin(
            output_directory, fname.replace(".bin", "-Difference-Map.png")
        )
        plt.imshow(diff)
    ...
    min_v = np.min(min_diff)
    max_v = np.max(max_diff)
    ...
    for diff_fname in diff_fnames:
        ...
        hist = h["histogram"]
        omin = h["omin"]
        cumu_h = np.cumsum(hist, dtype="float32")
        array_sz = diff.size
    ...
    out_fname = pjoin(
        output_directory, "Lambertian-Cumulative-Distribution-Differences.png"
    )
    fig = plt.figure()
    axis = fig.add_subplot(111)
...
def test_compare_brdf_files(
    ...
    # Output the histograms
    out_fname = pjoin(output_directory, "BRDF-Corrected-Histogram-Differences.png")
    fig = plt.figure()
    axis = fig.add_subplot(111)
    for bname in hist_results:
        y = hist_results[bname]
    ...
    out_fname = pjoin(
        output_directory, ("BRDF-Corrected-Cumulative-Distribution-" "Differences.png")
    )
    fig = plt.figure()
    axis = fig.add_subplot(111)
    for bname in cdist_results:
...
def test_compare_tc_files(
    ...
    # Output the histograms
    out_fname = pjoin(output_directory, "Terrain-Corrected-Histogram-Differences.png")
    fig = plt.figure()
    axis = fig.add_subplot(111)
    for bname in hist_results:
        y = hist_results[bname]
    ...
    out_fname = pjoin(
        output_directory,
        ("Terrain-Corrected-Cumulative-" "Distribution-Differences.png"),
    )
    fig = plt.figure()
    axis = fig.add_subplot(111)
    for bname in cdist_results:
...
"""
    ...
    md_template = template.format(
        lmbrt_data=lmbrt_data,
        lmbrt_diff_maps=lmbrt_diffs,
        brdf_data=brdf_data,
        brdf_diff_maps=brdf_diffs,
        tc_data=tc_data,
        tc_diff_maps=tc_diffs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_dir", required=True, help=("A directory path to the test output.")
    )
    parser.add_argument(
        "--pct_tolerance",
        default=3,
        type=int,
        help=("The tolerance for an acceptable percent " "difference."),
    )
    parser.add_argument(
        "--out_directory",
        required=True,
    ...
    suite.addTest(
        ParameterisedTestCase.parameterise(
            TestProductFileNames, reference_dir=reference_dir, test_dir=test_dir
        )
    )
