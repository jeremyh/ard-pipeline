...
try:
    ...
    subprocess = (
        process_manager.get_process_list_from_text(CONFIG.subprocess)
        or process_manager.get_process_list_from_file(CONFIG.subprocess_file)
        or process_manager.get_process_list_from_file(
            os.path.join(
                CONFIG.code_root, "image_processor", CONFIG.process_level + ".txt"
            )
        )
        or []
    )
    ...
    log_multiline(
        root_logger.error, traceback.format_exc(), "process failed: " + e.message, "\t"
    )
