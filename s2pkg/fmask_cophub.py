...
def fmask(dataset_path, task, out_fname):
    ...
    with tempfile.TemporaryDirectory(
        dir=os.path.basename(out_fname), prefix="pythonfmask-"
    ) as tmpdir:
        # filenames
        ...
        os.system(
            "unzip -p " + str(dataset_path) + " " + mtd_xml + " > " + zipfile_path
        )
        ...
        for key in img_dict.keys():
            command.append(
                " "
                + img_dict[key]["path"].replace("zip:", "/vsizip/").replace("!", "/")
            )
        ...
        # angles generation
        command = (
            "fmask_sentinel2makeAnglesImage.py -i "
            + zipfile_path
            + " -o "
            + angles_fname
        )
...
def main(output, datasets):
    ...
    for dataset in datasets:
        outpath = os.path.join(
            os.path.abspath(output), os.path.basename(dataset) + ".fmask"
        )
