...
def eval_sol_grids(lon_array, lat_array, time_array, day_of_year):
    ...
    latgc = np.arctan(np.tan(lat_array) * (1.0 - geodesic.earth.ECC2))

    logger.debug("GEOCENTRIC LAT = %s", latgc)

    # Solar zenith/elevation angle

    cos_sol_z = np.sin(latgc) * delta_sin + np.cos(latgc) * delta_cos * np.cos(h)
    sol_z = np.arccos(cos_sol_z)
    ...
    sol_az = np.arccos(
        (delta_sin - cos_sol_z * np.sin(latgc)) / (np.sin(sol_z) * np.cos(latgc))
    )
    # sol_az = numpy.where(h < 0, -sol_az, sol_az)
    np.sqrt(sol_az * sol_az, sol_az)
