...
class SlopeResultSet:
    ...
    def dump_arrays(
        self, output_path, l1t_input_dataset, file_type="ENVI", file_extension=".img"
    ):
...
def run_slope(
    ...
    assert dem_dat.shape == (nrow, ncol), (
        "dem_data not of correct shape "
        + str((nrow, ncol))
        + " != "
        + str(dem_dat.shape)
    )
    ...
    mask, theta, phit, it, et, azi_it, azi_et, rela, ierr = slope_pixelsize_newpole(
        ...
        is_utm,
        dem_dat,
        as_array(solar_zenith_data, dtype=np.float32),
        as_array(view_zenith_data, dtype=np.float32),
        as_array(solar_azimuth_data, dtype=np.float32),
        as_array(view_azimuth_data, dtype=np.float32),
    )
    ...
    def make_dataset(dataset, data):
        if type(dataset) == str:
            dataset = driver.Create(dataset, ncol, nrow, 1, DTYPE_MAP[np.float32])
