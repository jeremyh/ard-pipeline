    ...
    def __call__(cls, *args, **kwargs):
        if not cls._instance_for_singleton_ssfusousoifusos:
            with cls._lock_for_singleton_ssfusousoifusos:
                if not cls._instance_for_singleton_ssfusousoifusos:
                    cls._instance_for_singleton_ssfusousoifusos = super().__call__(
                        *args, **kwargs
                    )
...
def create_arg_string(func, *args, **kwargs):
    ...
    return ",\n\t".join(
        [
            "=".join([str(y) for y in item])
            for item in getcallargs(func, *args, **kwargs).iteritems()
        ]
    )
...
def print_call(logger):
    ...
    def wrap(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger(f"\ncalling {func.__module__}.{func.__name__}(\n\t{arg_str}\n)\n")
