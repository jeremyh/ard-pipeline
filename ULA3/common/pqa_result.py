...
class PQAResult(DataGrid):
    ...
    def __init__(self, image_array=None, filename=None, dtype="uint16"):
        """Constructor."""
        assert (
            image_array is not None or filename is not None
        )  # N.B: Filename will have priority over image_array
        ...
        try:
            if filename:  # Restore result array from existing file
                ...
                DataGrid.__init__(
                    self, array=np.zeros_like(image_array[0]).astype(np.dtype(dtype))
                )
                self.bitcount = 8 * self.array.itemsize
        finally:
    ...
    def set_mask(self, mask, bit_index, unset_bits=False):
        ...
        ), "Mismatched shape between mask and result arrays"
