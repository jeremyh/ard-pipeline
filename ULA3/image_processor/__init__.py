...
#            logger.debug('    _list[index] = %s', repr(_list[index]))
    ...
    if list_text:
        ...
        s = re.search(r'(.*\[)(.*?)(\].*)', _list_text) # Find innermost bracketed list (i.e. with no sublists)
        if s: # Bracketed list found
            if s.group(1) == '[' and s.group(3) == ']': # Flat bracketed list - don't process again
                ...
                sublist = get_process_list_from_text(re.sub(r'\[$', '', s.group(1)) +
                                                     placeholder +
                                                     re.sub(r'^\]', '', s.group(3)), level + 1)
    ...
    def __getattr__(self, attribute_name):
        ...
        def parse_list(list_string, element_type=str):
            return [element_type(element.strip())
                    for element in re.search(r'\s*(?<=\[)(.*)(?=\])\s*', list_string).group(0).split(',')]

        def parse_dict(list_string, key_type=str, value_type=str):
            result = dict()
            for key, value in [pair.strip().split(':') for pair in re.search(r'\s*(?<=\{)(.*)(?=\})\s*', list_string).group(0).split(',')]:
