...
def get_submodule_list(module_directory):
    ...
    """
    submodule_list = [
        path.splitext(path.basename(_lib))[0]
        for _lib in glob(path.join(module_directory, "[a-z]*.py"))
    ]
    # Append all valid package directories
    for pathname in [
        path.dirname(pathname)
        for pathname in glob(path.join(module_directory, "[a-z]*/__init__.py"))
    ]:
...
def get_process_list_from_text(list_text, level=0):
    ...
    def replace_placeholder(_list, placeholder, sublist):
        ...
        logger.debug(
            "  replace_placeholder(%s, %s, %s) called",
            repr(_list),
            repr(placeholder),
            repr(sublist),
        )
    ...
    logger.debug(
        "get_process_list_from_text(%s, %s) called", repr(list_text), repr(level)
    )
    if list_text:
        _list_text = re.sub(
            "#.*\n|\\s|\"|'", "", list_text
        )  # Remove comments, whitespaces and quotes
        s = re.search(
            r"(.*\[)(.*?)(\].*)", _list_text
        )  # Find innermost bracketed list (i.e. with no sublists)
        if s:  # Bracketed list found
            if (
                s.group(1) == "[" and s.group(3) == "]"
            ):  # Flat bracketed list - don't process again
                sublist = s.group(2).split(",")
            else:  # Nested list found - process again
                placeholder = "<sublist%s>" % (level)
                sublist = get_process_list_from_text(
                    re.sub(r"\[$", "", s.group(1))
                    + placeholder
                    + re.sub(r"^\]", "", s.group(3)),
                    level + 1,
                ...
                replace_placeholder(
                    sublist, placeholder, get_process_list_from_text(s.group(2))
                )
...
def get_subprocess_list(module_name):
    ...
    return get_process_list_from_file(
        path.join(CONFIG.code_root, module_name.replace(".", path.sep), "__init__.txt")
    )
...
def process(module_name, submodule_dict, subprocess_list=[], resume=False):
    ...
    def sequential_process(subprocess_list, resume):
        ...
        for subprocess in subprocess_list:
            ...
            if type(subprocess) == str:
                ...
                assert submodule_dict.get(
                    submodule
                ), "Sub-module %s does not exist in directory %s" % (
                    submodule,
                    module_name,
                )
                ...
                submodule_dict[submodule].process(
                    _subsubprocess_list, resume
                )  # Call individual submodule
    ...
    def parallel_process(subprocess_list, resume):
        ...
        for subprocess in subprocess_list:
            ...
            if type(subprocess) == str:
                ...
                logger.debug(
                    "Started thread for %s.process(%s, %s)",
                    submodule,
                    _subsubprocess_list,
                    resume,
                )
                ...
                process_thread.start()
                logger.debug(
                    "Started thread for process(%s, %s, %s, %s)",
                    module_name,
                    submodule_dict,
                    subprocess,
                    resume,
    ...
    assert (
        not resume or len(subprocess_list) <= 1
    ), "resume only valid when single or no subprocess specified"

    _subprocess_list = subprocess_list or get_subprocess_list(
        module_name
    )  # Use full process list if none provided
    logger.debug("_subprocess_list = %s", _subprocess_list)

    submodule = None
    if (
        len(_subprocess_list) == 1 and type(_subprocess_list[0]) == list
    ):  # [[process1, process2...processn]] indicates parallel processing is possible
        while len(_subprocess_list) == 1 and type(
            _subprocess_list[0]
        ):  # Use innermost nested list
...
class ProcessorConfig(object):
    ...
    _FORMATDICT = {
        datetime: ["%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M:%S.%f"],
        date: ["%Y-%m-%d"],
        time: ["%H:%M:%S", "%H:%M:%S.%f"],
    }
    ...
    def __init__(self, config_file=None):
        ...
        self.pqa_test_consts = (
            []
        )  # List containing all PQA test constant names defined in conf file
        self.pqa_test_index = (
            {}
        )  # Dict containing all PQA test bit indices keyed by constant names
        self.pqa_test_description = (
            {}
        )  # Dict containing all PQA test descriptions keyed by constant names
        self.pqa_param = (
            {}
        )  # Dict containing all PQA parameters and corresponding values
        ...
        def parse_args():
            ...
            _arg_parser.add_argument(
                "-f", "--fc", dest="fc_path", default=None, help="FC dataset directory"
            )
            ...
            _arg_parser.add_argument(
                "--l1t-root", dest="l1t_root", default=None, help="L1T root directory"
            )
            _arg_parser.add_argument(
                "--nbar-root",
                dest="nbar_root",
            ...
            _arg_parser.add_argument(
                "--pqa-root", dest="pqa_root", default=None, help="PQA root directory"
            )
            _arg_parser.add_argument(
                "--fc-root", dest="fc_root", default=None, help="FC root directory"
            )
            # _arg_parser.add_argument('--tc-root', dest='tc_root', default=None,
            #    help='TC root directory')

            _arg_parser.add_argument(
                "-w", "--work", dest="work_path", default=None, help="Work directory"
            )
            _arg_parser.add_argument(
                "-l", "--log", dest="log_file", default=None, help="Log File"
            )
            _arg_parser.add_argument(
                "--modtran-root",
                dest="_modtran_root",
            ...
            _arg_parser.add_argument(
                ...
                dest="li_source_description",
                default=None,
                help="Lineage source description",
            )
            _arg_parser.add_argument(
                "-p", "--purpose", dest="purpose", default=None, help="Purpose"
            )
            _arg_parser.add_argument(
                "-d",
                "--dataset-id",
        ...
        self.code_root = os.path.normpath(
        ...
        if not self._configParser:
            ...
            config_file = os.path.abspath(
                config_file
                or self._args.config_file
                or os.path.join(os.path.dirname(__file__), "processor.conf")
            )
    ...
    def open_config(self, config_file):
        ...
        def _read_option_types():
            ...
            assert self._configParser.has_section(
                "option_types"
            ), "Section [option_types] does not exist in config_file"

            for option in self._configParser.options("option_types"):
                option_type = ProcessorConfig._TYPEDICT.get(
                    self._configParser.get("option_types", option)
                )
        ...
        def _try_host_section(hostname):
            ...
            if hostname:
                if self._configParser.has_section(hostname):
                    ...
                    host_alias = re.match(r"(.*?)(\d*)(\.|$)", hostname).group(1)
        ...
        def _set_logging():
            ...
            if not self._configParser.has_section(
                "logging"
            ):  # Section [logging] does not exist in config_file
                return
            ...
            for module_name in self._configParser.options("logging"):
                log_level_string = self._configParser.get(
                    "logging", module_name
                ).upper()
                log_level = getattr(
                    logging, log_level_string
                )  # Convert from string to numeric
                assert isinstance(log_level, int), (
                    "Invalid logging level: " + log_level_string
                )

                module_logger = logging.getLogger("root." + module_name)
                module_logger.setLevel(log_level)
                logger.debug(
                    "    Logger %s set to log level %s",
                    module_logger.name,
                    log_level_string,
                )
        ...
        def read_ground_stations():
            assert self._configParser.has_section(
                "ground_stations"
            ), "No ground_stations section in conf file"

            for _ground_station in self._configParser.options("ground_stations"):
                self.station_code[_ground_station.upper()] = self._configParser.get(
                    "ground_stations", _ground_station
                )
            log_multiline(
                logger.debug, self.station_code, "Ground stations & codes: ", "\t"
            )
        ...
        def read_pqa_tests():
            ...
            assert self._configParser.has_section(
                "pqa_tests"
            ), "No pqa_tests section in conf file"

            saturation_dict = {}
            self.pqa_test_index["SATURATION"] = saturation_dict
            for _pqa_test_const in [
                _pqa_test_const.upper()
                for _pqa_test_const in self._configParser.options("pqa_tests")
            ]:
                # Get integer index and string description
                _index_desc = self._configParser.get(
                    "pqa_tests", _pqa_test_const
                ).split(",")
                _bit_index = int(_index_desc[0])
                _description = _index_desc[1]

                m = re.match(r"SATURATION_(\d{2})", _pqa_test_const)
            ...
            log_multiline(
                logger.debug, self.pqa_test_index, "PQA Tests & bit indices: ", "\t"
            )
            log_multiline(
                logger.debug, self.pqa_test_consts, "PQA Test constants: ", "\t"
            )
            log_multiline(
                logger.debug, self.pqa_test_description, "PQA Test descriptions: ", "\t"
            )

        def read_pqa_params():
            assert self._configParser.has_section(
                "pqa_params"
            ), "No pqa_params section in conf file"

            for _pqa_param in self._configParser.options("pqa_params"):
                self.pqa_param[_pqa_param.lower()] = self._configParser.getfloat(
                    "pqa_params", _pqa_param
                )
        ...
        def get_git_version():
            ...
            s = execute(
                ...
                ),  # __file__ should be <repository>/ULA3/image_processor/__init.py
                command_string="git show --pretty=oneline ",
            )["stdout"]

            if s and len(s) > 40:
                m = re.match(r"^(\w{40})\s+.*", s)
                if m:
                    result = m.group(1)
        ...
        if not _try_host_section(socket.gethostname()):
            if not _try_host_section(os.getenv("PBS_O_HOST", None)):
                ...
                host = os.getenv(
                    "PBS_JOBID", None
                )  # Try deriving cluster name from job ID
                if host:  # If this is a PBS job
                    host = re.match(r"(.*?).([a-z\-]+)pbs$", host).group(2)
                    host = "vayu" if host == "vu-" else host
                    _try_host_section(host)
                else:
                    _try_host_section(
                        "default_config"
                    )  # Fall back to universal Gdefault config section

        assert (
            self._section_name
        ), "No configuration section found for hostname or alias"
        ...
        if self.process_level == "nbar" or self.process_level == "tc":
            ...
            assert False, (
                "Invalid process "
                + self.process
                + " specified. Must be nbar, pqa, fc or tc."
            )
        ...
        for _level in self.input_levels:
            ...
            _input_root = vars(self._args).get(_level + "_root") or self.__getattr__(
                _level + "_data_root"
            )
            ...
            if (
                not os.path.exists(_input_path) and _input_path[0] != os.pathsep
            ):  # Try relative path
            ...
            if not self.work_path:
                ...
                if not self._args.work_path:
                    _work_path = os.path.join(
                        self.__getattr__("work_root"), _input_name
                    )
                else:
                    _work_path = self._args.work_path
                    if (
                        not os.path.exists(os.path.dirname(_work_path))
                        and self._args.work_path[0] != os.pathsep
                    ):
                        _work_path = os.path.join(
                            self.__getattr__("work_root"), _work_path
                        )

                _work_path = os.path.abspath(_work_path)
                assert os.path.exists(os.path.dirname(_work_path)), (
                    "Work path " + _work_path + " does not exist"
                )
        ...
        self.output_root = vars(self._args).get(
            self.output_level + "_root"
        ) or self.__getattr__(self.output_level + "_data_root")

        # Pre-pend root if nonexistent relative directory specified for output
        if self.output_path:
            if (
                not os.path.exists(os.path.dirname(self.output_path))
                and self.output_path[0] != os.pathsep
            ):
                self.output_path = os.path.join(self.output_root, self.output_path)

        if self.work_path:
            # Set up log file - defaults to process.log in work directory
            self.log_file = self._args.log_file or os.path.join(
                self.work_path, "process.log"
            )
    ...
    def __getattr__(self, attribute_name):
        ...
        def parse_list(list_string, element_type=str):
            return [
                element_type(element.strip())
                for element in re.search(r"\s*(?<=\[)(.*)(?=\])\s*", list_string)
                .group(0)
                .split(",")
            ]
        ...
        def parse_dict(list_string, key_type=str, value_type=str):
            ...
            for key, value in [
                pair.strip().split(":")
                for pair in re.search(r"\s*(?<=\{)(.*)(?=\})\s*", list_string)
                .group(0)
                .split(",")
            ]:
        ...
        try:  # Check command line arguments first
            ...
            if attribute_type == dict:
                ...
                for format_string in ProcessorConfig._FORMATDICT[attribute_type]:
                    try:
                        logger.debug(
                            "Trying format %s for %s object %s",
                            format_string,
        ...
        logger.debug(
            "  Cached configuration value: %s = %s", attribute_name, attribute_value
        )
