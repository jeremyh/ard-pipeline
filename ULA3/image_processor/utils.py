...
def create_output_image(
    DATA, CONFIG, rasterXSize=None, work_dir_prefix=None, image_postfix=None
):
    ...
    pfiles = [
        os.path.join(
            nbar_temp_output,
            "scene01",
            nbar_dataset_id + "_B" + str(band_file_number) + ".tif",
        )
        for band_file_number in l1t_input_dataset.satellite.rgb_bands
    ]
    ...
    browse_image_path = os.path.join(
        nbar_temp_output, nbar_dataset_id + image_postfix + ".jpg"
    )
    ...
    res = {
        "image_dim": create_image(
            pfiles, browse_image_path, thumbnail_work_dir, rasterXSize
        ),
...
def move_outputs(from_dir, to_dir):
    if from_dir == to_dir:
        logger.info(
            "Source and destinations directories the same (%s). No files moved",
            from_dir,
        )
        return
...
def calc_lat_long_grids(DATA, CONFIG):
    ...
    lon_grid = DataGrid(
        shape=l1t_input_dataset.shape,
        eval_func=l1t_input_dataset.geo_lon,
        depth=CONFIG.blrb_depth,
    )
    lat_grid = DataGrid(
        shape=l1t_input_dataset.shape,
        eval_func=l1t_input_dataset.geo_lat,
        depth=CONFIG.blrb_depth,
    )
    ...
    lon_grid.save_binary(
        filename=os.path.join(CONFIG.work_path, "LON.bin"),
        convert_angles=True,
        dtype=CONFIG.FORTRAN_GRID_DATATYPE,
    )
    lat_grid.save_binary(
        filename=os.path.join(CONFIG.work_path, "LAT.bin"),
        convert_angles=True,
        dtype=CONFIG.FORTRAN_GRID_DATATYPE,
    )
    ...
    if CONFIG.debug:
        lon_grid.save_image(
            filename=os.path.join(CONFIG.work_path, "LON.tif"), convert_angles=True
        )
        lat_grid.save_image(
            filename=os.path.join(CONFIG.work_path, "LAT.tif"), convert_angles=True
        )
...
def calc_satellite_grids(DATA, CONFIG):
    ...
    sat_ephem = satellite.load_tle(
        l1t_input_dataset.scene_centre_datetime,
        CONFIG.EPHEM_DATA_ROOT,
        CONFIG.TLE_SEARCH_RANGE,
    )
    assert sat_ephem, "Unable to load %s TLE for %s" % (
        satellite.NAME,
        l1t_input_dataset.scene_centre_datetime,
    )
    ...
    (time, sat_v, sat_az) = eval_sat_grids(
        lon_grid.array,
        lat_grid.array,
        centre_data,
        l1t_input_dataset,
        sat_ephem,
        CONFIG.view_angle_max,
        CONFIG.work_path,
        CONFIG.debug,
    )
    ...
    if CONFIG.debug:
        ...
        sat_v_grid.save_image(
            filename=os.path.join(CONFIG.work_path, "SAT_V.tif"), convert_angles=True
        )
        sat_az_grid.save_image(
            filename=os.path.join(CONFIG.work_path, "SAT_AZ.tif"), convert_angles=True
        )
        time_grid.save_image(
            filename=os.path.join(CONFIG.work_path, "TIME.tif"), convert_angles=False
        )
...
def calc_solar_grids(DATA, CONFIG):
    ...
    (sol_z, sol_az) = eval_sol_grids(
        lon_grid.array, lat_grid.array, time_grid.array, centre_data["day_of_year"]
    )
    ...
    sol_z_grid.save_binary(
        os.path.join(CONFIG.work_path, "SOL_Z.bin"),
        convert_angles=True,
        dtype=CONFIG.FORTRAN_GRID_DATATYPE,
    )
    rel_az_grid.save_binary(
        os.path.join(CONFIG.work_path, "REL_AZ.bin"),
        convert_angles=True,
        dtype=CONFIG.FORTRAN_GRID_DATATYPE,
    )
    ...
    if CONFIG.debug:
        # Save debug files to disk with angles in degrees
        sol_z_grid.save_image(
            os.path.join(CONFIG.work_path, "SOL_Z.tif"), convert_angles=True
        )
        rel_az_grid.save_image(
            os.path.join(CONFIG.work_path, "REL_AZ.tif"), convert_angles=True
        )
        DataGrid(array=sol_az).save_image(
            os.path.join(CONFIG.work_path, "SOL_AZ.tif"), convert_angles=True
        )
...
def calc_sat_sol_angle_grids(DATA, CONFIG):
    ...
    (
        ...
        X_cent,
        N_cent,
    ) = ca.calculate_angles(L1T_dataset, lon_arr, lat_arr, npoints=12)
    ...
    ut.write_img(
        satellite_zenith, sat_view_zenith_fname, projection=prj, geotransform=geoT
    )

    logger.debug("Writing satellite azimuth angle: %s", sat_azimuth_fname)
    ut.write_img(
        satellite_azimuth, sat_azimuth_fname, projection=prj, geotransform=geoT
    )

    logger.debug("Writing solar zenith angle: %s", solar_zenith_fname)
    ut.write_img(solar_zenith, solar_zenith_fname, projection=prj, geotransform=geoT)

    logger.debug("Writing solar azimith angle: %s", solar_azimuth_fname)
    ut.write_img(solar_azimuth, solar_azimuth_fname, projection=prj, geotransform=geoT)

    logger.debug("Writing relative azimuth angle: %s", relative_azimuth_fname)
    ut.write_img(
        relative_azimuth, relative_azimuth_fname, projection=prj, geotransform=geoT
    )
    ...
    create_centreline_file(Y_cent, X_cent, N_cent, cols, view_max=9.0, outdir=work_dir)
