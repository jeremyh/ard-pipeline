...
class DataManager(object):
    ...
    def __init__(self, work_path=None):
        ...
        self._item_dict = (
            {}
        )  # Class dict holding object instances keyed by (item_path, item_type) tuple
    ...
    def get_item(self, item_path, item_type, load_from_disk=True):
        ...
        with self._data_lock:
            ...
            if (
                item_instance is None and load_from_disk
            ):  # Try to load item from disk if not in memory
                try:
                    infile = None
                    ...
                    logger.warning(
                        "Unable to load %s object from file %s: %s",
                        item_type.__name__,
                        item_path,
                        e.message,
                    )
    ...
    def set_item(self, item_path, item_instance):
        ...
        logger.debug(
            "%s object %s set to %s in data_manager",
            item_type.__name__,
            item_path,
            item_instance,
        )
    ...
    def save_item(self, item_path, item_type=None, item_instance=None):
        ...
        ), "Either item_type or item_instance must be specified (but not both)"
        ...
        with self._data_lock:
            ...
            try:
                ...
                logger.info(
                    "%s object written to file %s", item_type.__name__, item_path
                )

                return item_instance
            except Exception, e:
                logger.warning(
                    "Unable to write %s object to file %s: %s",
                    item_type.__name__,
                    item_path,
                    e.message,
                )
    ...
    def free_item(self, item_path, item_type, save_first=False):
        ...
        if (item_path, item_type) in self._item_dict:
            ...
            with self._data_lock:
                ...
                logger.info(
                    "%s object %s deleted from data_manager",
                    item_type.__name__,
                    item_path,
                )
        else:
            assert not save_first, "Unable to save a non-existent %s object as %s" % (
                item_type.__name__,
                item_path,
            )
...
class DataGrid(object):
    ...
    def __init__(
        ...
        assert (
            (array is not None) or filename or (shape and (eval_func or dtype))
        ), "Insufficient information provided to instantiate DataGrid object"
    ...
    def interpolate(self, depth=1, shape=None, eval_func=None, dtype=numpy.float64):
        ...
        """
        logger.debug(
            "DataGrid.interpolate(%s, %s, %s, %s) called",
            depth,
            shape,
            eval_func,
            dtype,
    ...
    def save_binary(self, filename=None, convert_angles=False, dtype=None):
        ...
        logger.debug(
            "DataGrid.save_binary(%s, %s, %s) called",
            filename,
            convert_angles,
            repr(dtype),
        )
    ...
    def load_binary(self, filename=None, convert_angles=False, dtype=None):
        """ """
        logger.debug(
            "DataGrid.load_binary(%s, %s, %s) called", filename, convert_angles, dtype
        )
        filename = filename or self._filename or "GRID.dat"
    ...
    def save_image(
        ...
        """
        logger.debug(
            "DataGrid.save_image(%s, %s, %s, %s, %s, %s) called",
            filename,
            image_type,
            dtype,
            projection_ref,
        ...
        try:
            ...
            logger.debug(
                "Calling gdal_driver.Create(%s, %s, %s, %s, %s)",
                filename,
                out_array.shape[1],
                out_array.shape[0],
                1,
            ...
            gdal_dataset = gdal_driver.Create(
                filename, out_array.shape[1], out_array.shape[0], 1, gdal_dtype
            )
