...
class ReportMetadata(Metadata):
    ...
    _filename_pattern = r"report\.txt$"  # Default RegEx for finding metadata file.
...
"""
    ...
    def __init__(self, source=None):
        ...
        self._value_dict = (
            ReportMetadata._template_dict.copy()
        )  # Ensure that template will always work
        ...
        for line in [line for line in report_string.splitlines() if line]:
            ...
            if section == 0:
                if re.match("(Product Extent)|(PRODUCT FORMATTING)", line):
                    ...
                    if (
                        lh_name_value and rh_name_value
                    ):  # Two "name: value" expressions found on the same line
                ...
                if extent_y == 0 and re.match(r"\s*\|\s*\|\s*", line):
                    extent_y += 1
                    continue
                else:
                    for extent_x in range(2):  # Read in L & R values
                        name_value = get_name_value(
                            line[extent_x * 27 : (extent_x + 1) * 27 - 1]
                        )
                        if name_value:
    ...
    def write_file(self, filename=None, save_backup=False):
        ...
        try:
            ...
            assert outfile is not None, (
                "Unable to open Report file " + filename + " for writing"
            )
            ...
            if (
                "ORIENTATION" in self._value_dict.keys()
                and self._value_dict["ORIENTATION"] in ReportMetadata._orient_map.keys()
            ):
                self._value_dict["ORIENTATION"] = (
                    "%.1f deg"
                    % ReportMetadata._orient_map[self._value_dict["ORIENTATION"]]
                )
...
def main():
    r = ReportMetadata()
    r.read_file(
        "/home/alex/nbar/test_data/LS7_ETM_OTH_P51_GALPGS01_092_085_20100315/scene01/report.txt"
    )
