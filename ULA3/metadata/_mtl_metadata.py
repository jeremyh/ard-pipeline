...
class MTLMetadata(Metadata):
    ...
    _filename_pattern = r".*\.mtl"  # Default RegEx for finding metadata file.
...
END"""
    ...
    assert mtl_object.get_metadata(
        "L1_METADATA_FILE,PRODUCT_METADATA,SPACECRAFT_ID".split(",")
    ), "Unable to find value L1_METADATA_FILE,PRODUCT_METADATA,SPACECRAFT_ID"
    assert mtl_object.get_metadata(
        "...,SPACECRAFT_ID".split(",")
    ), "Unable to find value ...,SPACECRAFT_ID"
    assert not mtl_object.get_metadata(
        "RUBBERCHICKEN".split(",")
