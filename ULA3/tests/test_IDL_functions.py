...
class IDL_Hist_Tester(unittest.TestCase):
    def setUp(self):
        self.array1 = np.arange(10)
        self.control1 = self.array1 > 5
        self.array2 = np.arange(256)
        self.array3 = np.arange(10, 20, 0.5)
        self.array4 = np.random.ranf(1000)
        self.array5 = np.random.randint(0, 11, (100, 100))
        self.array6 = np.random.randint(0, 256, (256))

    def test_true_false_a(self):
        """Test that TRUE = 1 and FALSE = 0."""
        # Using an array 0->9, test how many are > 5
        bool_ = np.zeros((10), dtype="int8")
        _test_IDL_Histogram.test_bool(self.array1, 10, bool_)
        assert self.control1.sum() == 4
    ...
    def test_hist(self):
        ...
        assert h["histogram"].shape[0] == 256
        assert (h["histogram"] == 1).sum() == 256
    ...
    def test_hist_max(self):
        ...
        assert h["histogram"].shape[0] == 255
        assert (h["histogram"] == 1).sum() == 255
    ...
    def test_default_binsize(self):
        ...
        assert h["histogram"].shape[0] == 1
        # All values should be in the first bin.
        assert self.array4.shape[0] == h["histogram"][0]
    ...
    def test_reverse_indices1(self):
        ...
        element = np.random.randint(0, 256, (1))[0]
        bin = a[element]
        ri = h["ri"]
    ...
    def test_reverse_indices3(self):
        ...
        control = np.sort(a[(a >= 7.5) & (a < 17.5)])
        ri = h["ri"]
    ...
    def test_input3(self):
        ...
        b = self.array6
        h = IDL_Histogram(a, input=b)
        assert h["histogram"].shape[0] == b.shape[0]
