...
from ULA3.fc.utils import create_dir, datatype, unmix
...
def fractional_cover(
    nbar_data_path, asfloat32=False, fc_data_path=None, single_tif=False
):
    ...
    b_match = [
        band_no
        for band_no in nbar_dataset.bands("REFLECTIVE")
    ...
    r_data = np.zeros(
        (len(b_match), nbar_dataset.RasterYSize, nbar_dataset.RasterXSize), dtype=datype
    )
    ...
    #    r_data[wh] = null_val
    np.maximum(r_data, null_val, out=r_data)
    ...
    if fc_data_path:
        ...
        if single_tif:
            outds = driver.Create(
                outname, dims[1], dims[0], len(_fc_bands), gdal_datatype
            )
            ...
            for band_index in range(len(_fc_bands)):
                outds = driver.Create(
                    re.sub(r"\.tif$", "_" + _fc_bands[band_index][0] + ".tif", outname),
                    dims[1],
                    dims[0],
                    1,
                    gdal_datatype,
