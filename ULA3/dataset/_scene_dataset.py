    ...
    def Open(self, pathname, eAccess=gdalconst.GA_ReadOnly):
        ...
        def _gather_metadata():
            ...
            for metadata_type in ["XML", "MTL", "REPORT"]:
                for file_pattern in self._METADATA_SOURCES[metadata_type][
                    "FILE_PATTERNS"
                ]:
        ...
        def _set_instance_values():
            ...
            def get_mtl_bias_gain():
                ...
                mmr = self._metadata.get_metadata(
                    "MTL,L1_METADATA_FILE,MIN_MAX_RADIANCE"
                )
                if mmr:
                    # Get MIN_MAX_RADIANCE dict from MTL data
            ...
            if self.product_format.startswith(
                "FAST"
            ) and self.map_projection.startswith("EQR"):
                self.spatial_ref = osr.SpatialReference()
                self.spatial_ref.SetWellKnownGeogCS("WGS84")
                self.geotransform = [
                    self.ul_lon - self.pixel_x_size / 2.0,
                    #                    (self.ur_lon - self.ul_lon) / self.image_pixels,
                    self.pixel_x_size,
                    0.0,
                    self.ul_lat + self.pixel_y_size / 2.0,
                    0.0,
            ...
            self.scene_centre_x, self.scene_centre_y = self.coord_from_geo(
                self.lonlats["CENTRE"]
            )
                                ...
                                if suffix == root_suffix:
                                    ...
                                    try:  # ToDo: Need to deal with open failure here
                                        self._root_dataset = gdal.Open(
                                            self._root_dataset_pathname, eAccess
                                        )
                                    except (
                                        RuntimeError
                                    ), (
                                        e
                                    ):  # ToDo: Need to handle specific exception type
                                    ...
                                    thermal_dataset = gdal.Open(
                                        os.path.join(datadir, f), eAccess
                                    )

                                elif (
                                    extension == "FST" and suffix == "HPN"
                                ):  # Found Pan-Chromatic FST dataset
                                    panchromatic_dataset = gdal.Open(
                                        os.path.join(datadir, f), eAccess
                                    )
                ...
                if len(file_list) == 1:  # There can be only one...
                    ...
                    # FST has multiple rasters per subDataset
                        ...
                        # Two thermal bands in one dataset
                            ...
                            band_dict[
                                "rasterIndex"
                            ] = thermal_dataset.GetFileList().index(file_list[0])
                        ...
                        # Pan-chromatic band is in its own dataset
                            ...
                            band_file_number
                            in self.satellite.BAND_TYPES["PANCHROMATIC"]
                            and panchromatic_dataset
                        ):
