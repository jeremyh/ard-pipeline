...
class SceneDataset(Dataset):
    ...
    def __new__(
        ...
        if pathname and eAccess == gdalconst.GA_ReadOnly:
            return cls._CACHED_DATASETS.get(pathname) or super(
                SceneDataset, cls
            ).__new__(cls, pathname, eAccess, default_metadata_required, utm_fix)
        return super(SceneDataset, cls).__new__(
            cls, pathname, eAccess, default_metadata_required, utm_fix
        )
    ...
    def __init__(
        ...
        if (
            eAccess == gdalconst.GA_ReadOnly
            and pathname
            and SceneDataset._CACHED_DATASETS.get(pathname)
        ):
            return
        ...
        def _initialise_from_xml():
            ...
            class_config_xml_file = re.sub(r"\.py(c){0,1}$", ".xml", __file__)
            self._class_config_dom_tree = xml.dom.minidom.parse(class_config_xml_file)
            class_nodes = (
                self._class_config_dom_tree.documentElement.getElementsByTagName(
                    "CLASS"
                )
            )
            assert class_nodes, "Missing CLASS element in " + class_config_xml_file
            assert (
                unicode_to_ascii(class_nodes[0].getAttribute("NAME"))
                == self.__class__.__name__
            ), (
                "Invalid class name "
                + unicode_to_ascii(class_nodes[0].getAttribute("NAME"))
                + " in "
                + class_config_xml_file
            )
            class_node = class_nodes[0]
            data_nodes = class_node.getElementsByTagName("DATA")
            assert data_nodes, (
                "No DATA element defined under CLASS in " + class_config_xml_file
            )
            data_node = data_nodes[0]

            # Read in list of possible data directories (under root directory)
            datadir_nodes = data_node.getElementsByTagName("DATADIR")
            assert datadir_nodes, (
                "No DATADIR elements defined under DATA in " + class_config_xml_file
            )
            logger.debug("%s DATADIR nodes found", len(datadir_nodes))
            self._DATADIRS = []
            for datadir_node in datadir_nodes:
                self._DATADIRS.append(
                    unicode_to_ascii(datadir_node.getAttribute("PATH"))
                )
            logger.debug("self._DATADIRS = %s", self._DATADIRS)

            # Read in list of possible data directories (under root directory)
            filetype_nodes = data_node.getElementsByTagName("FILETYPE")
            assert filetype_nodes, (
                "No FILETYPE element defined under DATA in " + class_config_xml_file
            )
            self._FILE_TYPE_INFO = {}
            for filetype_node in filetype_nodes:
                extension = unicode_to_ascii(filetype_node.getAttribute("EXTENSION"))
                assert extension, (
                    "No EXTENSION attribute defined under FILETYPE in "
                    + class_config_xml_file
                )
                root_suffix = unicode_to_ascii(filetype_node.getAttribute("ROOTSUFFIX"))
                assert root_suffix, (
                    "No ROOTSUFFIX attribute defined under FILETYPE in "
                    + class_config_xml_file
                )
                product_format = unicode_to_ascii(
                    filetype_node.getAttribute("PRODUCTFORMAT")
                )
                assert product_format, (
                    "No PRODUCTFORMAT attribute defined under FILETYPE in "
                    + class_config_xml_file
                )
                self._FILE_TYPE_INFO[extension] = {
                    "ROOTSUFFIX": root_suffix,
            ...
            assert metadata_nodes, (
                "No METADATA_SOURCE element defined under CLASS in "
                + class_config_xml_file
            )
            ...
            for metadata_node in metadata_nodes:
                ...
                metadata_required = self.get_bool(
                    unicode_to_ascii(metadata_node.getAttribute("REQUIRED"))
                )  # Defaults to False
                ...
                assert metadata_file_nodes, (
                    "No FILE elements defined under "
                    + metadata_type
                    + " in "
                    + class_config_xml_file
                )
                ...
                for metadata_file_node in metadata_file_nodes:
                    file_patterns.append(
                        unicode_to_ascii(metadata_file_node.getAttribute("PATTERN"))
                    )
                assert file_patterns, (
                    "No file matching pattern(s) defined for "
                    + metadata_type
                    + " in "
                    + class_config_xml_file
                )
        ...
        try:
            ...
            self._panchromatic_dataset = (
                None  # Current panchromatic_dataset (gdal.Dataset)
            )
            self._data_dir = None  # Directory containing image files
            self._sub_dataset_type = None  # 'TIF' or 'FST'
            self._xml_uses_attributes = (
                False  # Flag indicating whether XML values are stored as tag attributes
            )
            ...
            self.pq_tests_run = (
                None  # Tests run for PQ datasets, flags in the form of a 16 bit integer
            )
    ...
    def __gather_metadata(self):
        ...
        for metadata_type in ["XML", "MTL", "REPORT"]:
            for file_pattern in self._METADATA_SOURCES[metadata_type]["FILE_PATTERNS"]:
                ...
                if file_list:
                    logger.debug(
                        "Reading %s metadata file %s", metadata_type, file_list[0]
                    )
                    if metadata_type == "XML":
                        xml_metadata = XMLMetadata(file_list[0])
                        self._xml_uses_attributes = (
                            xml_metadata.uses_attributes
                        )  # Remember whether attributes are used in XML
                        self._metadata.merge_root_metadata_from_object(
                            metadata_object=xml_metadata, overwrite=False
                        )
                    elif metadata_type == "MTL":
                        self._metadata.merge_root_metadata_from_object(
                            metadata_object=MTLMetadata(file_list[0])
                        )
                    elif metadata_type == "REPORT":
                        self._metadata.set_root_metadata_from_object(
                            metadata_object=ReportMetadata(file_list[0])
                        )
                else:
                    logger.debug(
                        'No %s files matching "%s" found', metadata_type, file_pattern
                    )

            assert (
                self._metadata.get_metadata([metadata_type])
                or not self._METADATA_SOURCES[metadata_type]["REQUIRED"]
        ...
        assert self.satellite, "Unable to create Satellite object for %s %s" % (
            self.satellite_name,
            self.sensor,
        )
            ...
            if os.path.isdir(datadir):
                for f in sorted(os.listdir(datadir)):
                    ...
                    if m:
                        ...
                        if extension == "TIF":
                            self._root_dataset_pathname = os.path.abspath(
                            ...
                            try:
                                self._root_dataset = gdal.Open(
                                    self._root_dataset_pathname, self._eAccess
                                )
    ...
    def __load_bands_fc(self):
        ...
        for band_index in range(len(self._FC_BAND_LIST)):
            ...
            file_list = glob(
                os.path.join(
                    self._data_dir,
                    "*_" + band_suffix + "." + self._sub_dataset_type.lower(),
                )
            )
            file_list += glob(
                os.path.join(
                    self._data_dir,
                    "*_" + band_suffix + "." + self._sub_dataset_type.upper(),
                )
            )
            if len(file_list) == 1:
                ...
                logger.debug(
                    "%s files found for band file number %s"
                    % (len(file_list), band_file_number)
                )
        ...
        if len(self._raster_dict) < len(self._FC_BAND_LIST):
            logger.debug(
                "Only %s/%s bands found"
                % (len(self._raster_dict), len(self._FC_BAND_LIST))
            )
    ...
    def __find_root_dataset_xml(self):
            ...
            if os.path.isdir(datadir):
                for f in sorted(os.listdir(datadir)):
                    ...
                    if m:
                        ...
                        logger.debug(
                            "    extension in _FILE_TYPE_INFO = %s",
                            (extension in self._FILE_TYPE_INFO),
                        )
                        logger.debug(
                            "    satellite.root_band = %s", self.satellite.root_band
                        )
                        ...
                        if extension in self._FILE_TYPE_INFO:
                            try:
                                root_suffix = (
                                    self._FILE_TYPE_INFO[extension]["ROOTSUFFIX"]
                                    % self.satellite.root_band
                                )
                            except TypeError:
                                root_suffix = self._FILE_TYPE_INFO[extension][
                                    "ROOTSUFFIX"
                                ]
                            ...
                            if suffix == root_suffix:
                                ...
                                logger.debug(
                                    "    _root_dataset_pathname = %s",
                                    self._root_dataset_pathname,
                                )
                                ...
                                try:  # ToDo: Need to deal with open failure here
                                    ...
                                    RuntimeError
                                ), e:  # ToDo: Need to handle specific exception type
                                    self._root_dataset = None
                                    raise DSException(e.message)
                                ...
                                extension == "FST" and suffix == "HTM"
                            ):  # Found Thermal FST dataset
                                self._thermal_dataset = gdal.Open(
                                    os.path.join(datadir, f), self._eAccess
                                )

                            elif (
                                extension == "FST" and suffix == "HPN"
                            ):  # Found Pan-Chromatic FST dataset
                                self._panchromatic_dataset = gdal.Open(
                                    os.path.join(datadir, f), self._eAccess
                                )

            if (
                self._data_dir and self._root_dataset_pathname
            ):  # Data directory has been determined
                break  # Stop searching
        ...
        logger.debug(
            "    self._root_dataset_pathname = %s", self._root_dataset_pathname
        )
        logger.debug("")

        assert self._data_dir and self._root_dataset_pathname, (
            "Unable to find root dataset under " + self._pathname
        )
    ...
    def __load_bands_xml(self):
        ...
        for band_index in range(len(self.satellite.BAND_LIST)):
            ...
            file_list = glob(
                os.path.join(
                    self._data_dir,
                    "*_B"
                    + str(band_file_number)
                    + "."
                    + self._sub_dataset_type.lower(),
                )
            )
            file_list += glob(
                os.path.join(
            ...
            if len(file_list) == 1:  # There can be only one...
                ...
                if self._sub_dataset_type == "TIF":
                    ...
                    if band_dict["file"] == self._root_dataset_pathname:
                        ...
                        band_dict["dataSet"] = gdal.Open(
                            band_dict["file"], self._eAccess
                        )
                ...
                # FST has multiple rasters per subDataset
                    ...
                    if (
                        band_file_number in self.satellite.BAND_TYPES["REFLECTIVE"]
                        and self._root_dataset
                    ):
                        # Filter *.aux.xml files from the root dataset file list.
                        # These XML files mess up band_dict construction.
                        __flist = [
                            x
                            for x in self._root_dataset.GetFileList()
                            if not x.endswith(".aux.xml")
                        ]
                    ...
                    # Two thermal bands in one dataset
                        ...
                        band_file_number in self.satellite.BAND_TYPES["THERMAL"]
                        and self._thermal_dataset
                    ):
                        band_dict["dataSet"] = self._thermal_dataset
                        band_dict[
                            "rasterIndex"
                        ] = self._thermal_dataset.GetFileList().index(file_list[0])
                    ...
                    # Pan-chromatic band is in its own dataset
                        ...
                        band_file_number in self.satellite.BAND_TYPES["PANCHROMATIC"]
                        and self._panchromatic_dataset
                    ):
                        ...
                        raise Exception(
                            "Invalid band file number " + str(band_file_number)
                        )
        ...
        if len(self._raster_dict) < len(self.satellite.BAND_TYPES["ALL"]):
            logger.debug(
                "Only %s/%s bands found"
                % (len(self._raster_dict), len(self.satellite.BAND_TYPES["ALL"]))
            )

        # Set up rgb bands
        if self.satellite.rgb_bands:
            self.rgb_bands = [
                self._band_number_map[band_file_number]
                for band_file_number in self.satellite.rgb_bands
            ]
    ...
    def __set_instance_values(self):
        ...
        if self.scene_centre_date and self.scene_centre_time:
            ...
            self.scene_start_datetime = (
                self.scene_centre_datetime - self.satellite.acquistion_seconds // 2
            )
            self.scene_end_datetime = (
                self.scene_start_datetime + self.satellite.acquistion_seconds
            )
        else:
        ...
        if self.product_format.startswith("FAST") and self.map_projection.startswith(
            "EQR"
        ):
            self.spatial_ref = osr.SpatialReference()
            ...
            self.geotransform = (numpy.array(gt) / 100000.0).tolist()
            ...
            logger.warning(
                "FAST-EQR format, created substitute spatial reference and geotransform"
            )
        ...
        if _local:
            assert self.utm_fix, (
                "LOCAL_CS [%s] (UTM zone could be outside the datum spec) -- ABORTED"
                % _local
            )

            m = re.search(r"UTM Zone (\d+) ", _local)  # hardwired
            if m:
                zone = int(m.group(1))
            else:
            ...
            logger.warning(
                "Overrode LOCAL_CS spatial reference with WGS84 (zone=%d)" % zone
            )
        ...
        self.cxform_to_geo = osr.CoordinateTransformation(
            self.spatial_ref, self.spatial_ref_geo
        )
        self.cxform_from_geo = osr.CoordinateTransformation(
            self.spatial_ref_geo, self.spatial_ref
        )
        ...
        if (
            self.zone
            and self.zone < 0
            and self.datum == "GDA94"
            and self.earth_ellipsoid == "GRS80"
        ):
    ...
    def __parse_pq_tests_run(self):
        ...
        if self.dq_statement:
            bit_string_list = re.findall(
                r"\(Bit (\d+)\): (Run|Not Run)", self.dq_statement
            )
            pq_tests_run = 0
    ...
    def __get_mtl_bias_gain(self):
        ...
        if mmr:
            # Get MIN_MAX_RADIANCE dict from MTL data
            mmpv = self._metadata.get_metadata(
                "MTL,L1_METADATA_FILE,MIN_MAX_PIXEL_VALUE"
            )
            if mmpv:
                # TODO: This will probably break with Landsat 8 data
                for key in sorted(mmr.keys()):
                    m = re.match(r"LMIN_BAND(\d+)", key)
                    if m:
                        ...
                        assert band_file_number in self.satellite.BAND_TYPES["ALL"], (
                            "Invalid band file number %d" % band_file_number
                        )
                ...
                logger.debug(
                    "No MIN_MAX_PIXEL_VALUE data found in MTL file. Unable to compute bias & gain."
                )
        else:
            logger.debug(
                "No MIN_MAX_RADIANCE data found in MTL file. Unable to compute bias & gain."
            )
    ...
    def __get_mtl_bias_gain_landsat8_lookup(self):
        ...
        rad_rescale_params = self._metadata.get_metadata(
            "MTL,L1_METADATA_FILE,RADIOMETRIC_RESCALING"
        )
        ...
        if rad_rescale_params:
            for key, value in rad_rescale_params.iteritems():
                ...
                match_add = re.match(r"RADIANCE_ADD_BAND_(\d+)", key)
                if match_add:
                    bdict[int(match_add.group(1))] = float(value)

                # Gain: RADIANCE_MULT_BAND_X = <value>

                match_mult = re.match(r"RADIANCE_MULT_BAND_(\d+)", key)
                if match_mult:
                    gdict[int(match_mult.group(1))] = float(value)
        else:
            logger.debug(
                "No RADIOMETRIC_RESCALING data found in MTL file. Unable to compute bias & gain."
            )
    ...
    def read_metadata(
        ...
        """
        logger.debug(
            "read_metadata(%s, %s, %s) called",
            repr(class_config_dom_tree),
            repr(metadata_type_id),
            repr(ignore_empty_values),
        )
        ...
        try:
            ...
            for variable_node in variable_nodes:
                ...
                var_type = self.__class__._TYPE_MAP.get(
                    variable_node.getAttribute("TYPE"), str
                )
                logger.debug("  Variable type = %s", repr(var_type))

                # Optional REQUIRED attribute - defaults to True
                var_required = self.get_bool(
                    unicode_to_ascii(variable_node.getAttribute("REQUIRED")),
                ...
                assert var_metatada_nodes, (
                    "No metadata search paths defined for " + var_name
                )
                for var_metatada_node in var_metatada_nodes:
                    ...
                    if metadata_type_id and not re.match(
                        "^" + metadata_type_id + ",", var_key
                    ):
                        continue

                    var_string = self._metadata.get_metadata(
                        var_key.split(",")
                    )  # Search for metadata

                    if (
                        type(var_string) == dict
                    ):  # Incomplete metadata path - not a leaf node
                        var_string = None

                    if (
                        var_string is not None
                    ):  # Value not found in metadata (empty string allowed)
                        # Check for optional regex and even more optional group
                        var_regex = (
                            unicode_to_ascii(var_metatada_node.getAttribute("REGEX"))
                            or None
                        )
                        var_regex_group = 0
                        if var_regex:
                            try:
                                var_regex_group = int(
                                    unicode_to_ascii(
                                        var_metatada_node.getAttribute("GROUP")
                                    )
                                )
                            except ValueError:
                                var_regex_group = 0
                            ...
                            logger.debug(
                                "  RegEx processed metadata string = %s",
                                repr(var_string),
                            )
                ...
                assert var_string or ignore_empty_values or not var_required, (
                    var_name + " not found in metadata"
                )
                ...
                if not var_string:
                    ...
                    var_format = unicode_to_ascii(
                        var_metatada_node.getAttribute("FORMAT")
                    )
                    assert (
                        var_format
                    ), "Format string must be provided for datetime/date/time metadata"
                    try:
                        logger.debug(
                            "  Trying datetime format string = %s", repr(var_format)
                        )
                    ...
                    assert (
                        var_value or ignore_empty_values or not var_required
    ...
    def update_metadata(
        ...
        """
        logger.debug(
            "update_metadata(%s, %s, %s, %s, %s, %s) called",
            repr(var_name_or_var_name_list),
            repr(first_match_only),
            repr(create_new_nodes),
            repr(metadata_type_id),
            repr(class_config_dom_tree),
        ...
        try:
            ...
            for variable_node in variable_nodes:
                ...
                for var_metatada_node in var_metatada_nodes:
                    ...
                    assert type(old_var_string) != dict, (
                        "Incomplete metadata path " + var_key + " (not a leaf node)"
                    )  # Could be None
                    ...
                    assert var_type == type(var_value) or var_value is None, (
                        "Variable type mismatch for "
                        + var_name
                        + ". (Expected "
                        + var_type.__name__
                        + ", found "
                    ...
                    if var_value is None:
                        ...
                        logger.debug(
                            "  Formatting date/time %s using format string %s",
                            repr(var_value),
                            repr(var_format),
                        )
                    ...
                    logger.debug(
                        "  Metadata string %s = %s", var_name, repr(var_string)
                    )
                    ...
                    if var_regex and old_var_string:
                        ...
                        if (
                            not s
                        ):  # Don't change string if we can't find a match in the old one
                            logger.debug(
                                'Regex "%s" not found in old string "%s"',
                                var_regex,
                                old_var_string,
                            )
                            continue
                        ...
                        try:
                            var_string = (
                                old_var_string[0 : s.start(var_regex_group)]
                                + var_string
                                + old_var_string[s.end(var_regex_group) :]
                            )
                            logger.debug(
                                "  Substituted metadata string %s = %s",
                                var_name,
                                repr(var_string),
                            )
                        except IndexError:
                            logger.debug(
                                'Regex group "%s" not found in search result "%s"',
                                var_regex_group,
                                s.groups(),
                            )
                            continue
                    ...
                    if var_string != old_var_string and not (
                        keep_existing_values and old_var_string
                    ):
                        self._metadata.set_metadata_node(
                            var_key.split(","), var_string, overwrite=True
                        )
                        logger.debug("  Metadata %s set to %s", var_key, var_string)

                        if not vars_changed.get(
                            var_name
                        ):  # No var_key list defined for var_name
                            vars_changed[var_name] = [
                                var_key
                            ]  # Create new var_key list
                        else:
                            vars_changed[var_name].append(
                                var_key
                            )  # Append var_key to existing list

            logger.debug(
                "Specified variables found in metadata : %s", sorted(list(vars_found))
            )
            logger.debug(
                "Specified variables changed in metadata : %s",
                sorted(vars_changed.keys()),
            )
            ...
            assert not vars_not_found, (
                str(len(vars_not_found))
                + " specified variables not found in metadata : "
                + str(sorted(list(vars_not_found)))
            )

            logger.info(
                "%s / %s values changed in metadata", len(vars_changed), len(vars_found)
            )
        except (AssertionError, DSException), e:
    ...
    def bounds_getter(self):
        ...
        def gtr(dataset):
            ...
            return ImageShape(
                ...
                g[1],
                g[5],
                dataset.spatial_ref.ExportToWkt(),
            )
    ...
    def GetFileList(self):
        ...
        return list(
            self._file_list
        )  # Return a copy of the private file list to prevent interference with it
    ...
    def ReadAsArray(self, *args, **kwargs):
        ...
        def get_array(dataset, template_ds):
            if (
                dataset.RasterXSize == template_ds.RasterXSize
                and dataset.RasterYSize == template_ds.RasterYSize
            ):
                return dataset.ReadAsArray()
            else:
                logger.debug(
                    "  Reprojecting %s to the same resolution as %s",
                    dataset.GetFileList()[0],
                    template_ds.GetFileList()[0],
                )
                template_band = template_ds.GetRasterBand(1)
                driver = gdal.GetDriverByName("MEM")
                mem_dataset = driver.Create(
                    "",
        ...
        if self._sub_dataset_type == "FST":
            ...
            datasets = [
                bd["dataSet"]
                for bd in self._raster_dict.values()
                if bd["band_file_number"]
                not in self.satellite.BAND_TYPES["PANCHROMATIC"]
            ]
            # Construct 3D array from first non-panchromatic subdataset
            result_array = numpy.resize(
                datasets[0].ReadAsArray(),
                (len(datasets), datasets[0].RasterYSize, datasets[0].RasterXSize),
            )
    ...
    def SetMetadataItem(self, name, value, domain=""):
        """Overrides Dataset method"""
        if self._eAccess != gdalconst.GA_ReadOnly:
            self._metadata.set_metadata_value(
                domain.split(",") + name.split(","), value, True
            )
    ...
    def img_coord(self, pixel, line, d_pixel=0.5, d_line=0.5):
        ...
        return (
            (g[0] + g[1] * pixel + g[2] * line) + g[1] * d_pixel,
