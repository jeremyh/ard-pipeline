...
class Satellite(object):
    ...
    def __init__(self, sat_name, sensor):
        ...
        def get_sat_attributes(sat_node):
            ...
            assert self.TAG in [
                ...
                "LS7",
                "LS8",
            ], "Unhandled satellite tag: " + repr(self.TAG)

            # Orbital semi-major axis (metres)
            self.SEMI_MAJOR_AXIS = float(
                unicode_to_ascii(sat_node.getAttribute("SEMI_MAJOR_AXIS"))
            )
            ...
            self.INCLINATION = float(
                unicode_to_ascii(sat_node.getAttribute("INCLINATION"))
            )
            ...
            self.SWEEP_PERIOD = float(
                unicode_to_ascii(sat_node.getAttribute("SWEEP_PERIOD"))
            )
            ...
            self.SOLAR_IRRAD_FILE = unicode_to_ascii(
                sat_node.getAttribute("SOLAR_IRRAD_FILE")
            )

            # Spectral filter file
            self.SPECTRAL_FILTER_FILE = unicode_to_ascii(
                sat_node.getAttribute("SPECTRAL_FILTER_FILE")
            )

            # POOMA number - approximation only
            self.NOMINAL_PIXEL_DEGREES = float(
                unicode_to_ascii(sat_node.getAttribute("NOMINAL_PIXEL_DEGREES"))
            )
            ...
            self.CLASSIFICATION = unicode_to_ascii(
                sat_node.getAttribute("CLASSIFICATION")
            )

            # International designator
            # TLE compatible format: <last two digits of launch year><launch number of the year><piece of the launch>
            self.INTL_DESIGNATOR = unicode_to_ascii(
                sat_node.getAttribute("INTL_DESIGNATOR")
            )
        ...
        def get_sensor(sat_node, sensor):
            def get_bands(sensor_node):
                for band_node in sensor_node.getElementsByTagName("BAND"):
                    band_dict = {}
                    band_number = int(
                        unicode_to_ascii(band_node.getAttribute("NUMBER"))
                    )
                    ...
                    band_dict["WAVELENGTH"] = [
                        float(wavelength)
                        for wavelength in unicode_to_ascii(
                            band_node.getAttribute("WAVELENGTH")
                        ).split("-")
                    ]
                    ...
                    band_dict["RESOLUTION"] = float(
                        unicode_to_ascii(band_node.getAttribute("RESOLUTION"))
                    )
            ...
            for sensor_node in sat_node.getElementsByTagName("SENSOR"):
                ...
                if sensor_name.lower() == sensor.lower():
                    ...
                    self.k = (
                        float(unicode_to_ascii(sensor_node.getAttribute("K1"))),
            ...
            self.sensor_description = unicode_to_ascii(
                sensor_node.getAttribute("DESCRIPTION")
            )
            self.root_band = int(
                unicode_to_ascii(sensor_node.getAttribute("ROOT_BAND"))
            )
            self.rgb_bands = [
                int(bandstring)
                for bandstring in unicode_to_ascii(
                    sensor_node.getAttribute("RGB_BANDS")
                ).split(",")
            ]
            self.acquistion_seconds = timedelta(
                seconds=float(
                    unicode_to_ascii(sensor_node.getAttribute("ACQUISITION_SECONDS"))
                )
            )
        ...
        if (
            not Satellite._dom_tree
        ):  # If the satellite.xml file hasn't already been opened
        ...
        assert self.NAME_PATTERN, (
            "Configuration for " + sat_name + " does not exist in " + self.config_file
        )
    ...
    def load_tle(self, centre_datetime, data_root, date_radius=45):
        ...
        return self._load_tle_from_archive(
            centre_datetime, data_root, date_radius
        ) or self._load_tle_from_file_sequence(centre_datetime, data_root, date_radius)
    ...
    def _load_tle_from_archive(self, centre_datetime, data_root=None, date_radius=45):
        ...
        def _cmp(x, y):
            return abs(x) - abs(y)  # For sorting date offsets.

        date_deltas = [
            timedelta(days=d)
            for d in sorted(range(-date_radius, date_radius), cmp=_cmp)
        ]
        yyddd_list = [
            x.strftime("%02y%03j") for x in [(centre_datetime + d) for d in date_deltas]
        ]
        ...
        for yyddd in yyddd_list:
            _smap["YYDDD"] = yyddd
            m = re.search(
                TLE_ENTRY_PATTERN_FORMAT % _smap, tle_archive_text, re.MULTILINE
            )
            if m:
        ...
        logger.error(
            "No TLE found for %s in archive file %s"
            % (centre_datetime, tle_archive_path)
        )
        return None

    def _load_tle_from_file_sequence(
        self, centre_datetime, data_root=None, tle_search_range=45
    ):
        ...
        data_subdir = re.sub(r"\W", "", self.NAME).upper()
        ...
        if os.path.exists(tle_path):
            try:
                ...
                logger.warning(
                    "Unable to open primary TLE file %s: %s", tle_path, e.message
                )
        ...
        for d in xrange(1, tle_search_range):
            ...
            for s in (-1, 1):
                ...
                tle_dir = os.path.join(
                    data_root, data_subdir, "TLE", "%s_YEAR" % self.TAG, "%4d" % dt.year
                )
                ...
                if os.path.exists(tle_path):
                    try:
                        ...
                        Exception
                    ), e:  # TODO: Tighten up this except clause - too general
                        logger.warning(
                            "Unable to open Secondary TLE file %s: %s",
                            tle_path,
                            e.message,
                        )

        logger.error(
            "No valid TLE file found for %s in %s", dt.strftime("%Y-%m-%d"), tle_dir
        )
        return None
...
def eval_centre_data(scene_dataset, sat_ephem):
    ...
    centre_data = {
        ...
        "day_of_year": doy,
        "decimal_hour": dec_datetime.hour_fraction(scene_dataset.scene_centre_datetime)
...
def compute_time_samples(
    ...
    logger.debug(
        "compute_time_samples(%s, %s, %s, %s, %s, %s, %s, %s) called",
        lon_array,
        lat_array,
        centre_data,
        l1t_input_dataset,
    ...
    sat_ephem.compute(
        (
            centre_time.year,
            centre_time.month,
            dec_datetime.decimal_day_of_month(centre_time),
        )
    )
    ...
    for i in xrange(-nc / 2, nc / 2):
        ...
        x, y, _h = l1t_input_dataset.cxform_from_geo.TransformPoint(
            math.degrees(lon), math.degrees(lat), 0
        )
        ...
        skew = math.atan2(
            earth.OMEGA * cos_glat * math.cos(beta),
    ...
    for disp_key, disp_az_delta in disp_map.iteritems():
        ...
        while len(t_used) < len(times):
            ...
            for t in sorted(t_list):
                ...
                (centre_arc_distance, az_to_centre, _az_from_centre) = vinc_dist(
                    earth.F, earth.A, lat, lon, clat, clon
                )
                ...
                q = math.sin(latgc) * math.sin(clatgc) + math.cos(latgc) * math.cos(
                    clatgc
                ) * math.cos(dlon)
                ...
                skew = math.atan2(
                    earth.OMEGA * cos_latgc * math.cos(beta),
            ...
            if out_of_bounds:
                ...
                if out_pass_count > OUT_OF_BOUNDS_DISPLACEMENT_PASSES:
                    logger.info(
                        "%s PROPAGATION STOPPED (count = %d)", disp_key, disp_count
                    )
                    break
    ...
    for t in sorted(centre_pts):
        ...
        s_pt = (
            "%f, %f, %e, %e, %e, %e # " "d=%f, gs=%f gh=%f ga=%f az=%f azv=%f # %s %s"
        ) % (
            math.degrees(lon),
            math.degrees(lat),
            x,
    ...
    logger.debug(
        "TIME AND ANGLE SAMPLE POINTS (n=%d, buffer_size=%d)",
        len(sample_pts_list),
        n_samples_buf,
    )
    logger.debug(
        "PARAMETERS: nc = %d, delta_tc = %f, delta_rc = %f", nc, delta_tc, delta_rc
    )
...
def create_centre_line_file(
    ...
    """
    logger.debug(
        "create_centre_line_file(%s, %s, %s, %s, %s, %s) called",
        clpts_tuple,
        shape,
        x_range,
        y_range,
    ...
    logger.info(
        "interpolating centre line samples using scipy.interpolate.splrep/splrev"
    )
    ...
    if abs(delta_beg) > tpoint_delta_tol or abs(delta_end) > tpoint_delta_tol:
        kdeg = 3
        logger.info(
            "interpolating centre line samples using numpy.polyfit (deg=%d)", kdeg
        )
...
def interpolate_sample_points(xy, f, mg, method="linear", fill_value=numpy.nan):
    ...
    """
    logger.debug(
        "interpolate_sample_points(%s, %s, %s, %s, %s) called",
        xy,
        f,
        mg,
        method,
...
def eval_sat_grids(
    ...
    cl_indices = create_centre_line_file(
        cline_points,
        lon_array.shape,
        xgr,
        ygr,
        l1t_input_dataset.cxform_to_geo,
        view_angle_max,
        clfpath,
    )
    ...
    for key in vu_grids:
        ...
        if debug:
            logger.debug(
                "VIEW ANGLE: %s %s %s", key, vu_grids[key].shape, vu_grids[key]
            )
        del grid
