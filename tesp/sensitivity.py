...
def assert_ints(array):
    ...
    assert np.allclose(array, np.around(array))
    return np.around(array).astype("int32")
...
class GeoBox:
    ...
    def __or__(self, other):
        ...
        """
        assert np.allclose(self.linear_part(), other.linear_part())

        translation = other.shift_part() - self.shift_part()
        linear = self.linear_part()
        steps = assert_ints(np.linalg.solve(linear, translation))

        shift = np.array(self.affine * np.where(steps > 0, 0, steps))
        affine = Affine(
        ...
        shape = (
            int(max(self_end[1], other_end[1])),
            int(max(self_end[0], other_end[0])),
        )
