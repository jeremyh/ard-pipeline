...
def unpack(input_file, outdir):
    ...
    def unpack_dataset(product, product_name, band):
        ...
        outfile = pjoin(
            outdir, "{}_{}.tif".format(product_name, dataset.attrs["alias"])
        )
        ...
        if (band + "_pixelcount") not in product:
            ...
            write_img(data, outfile, nodata=nodata, geobox=geobox, options=options)
...
class Experiment(luigi.Task):
    ...
    def requires(self):
        ...
        tree = dict_tree(
            [exp for future in as_completed(futures) for exp in future.result()],
            ["mean_sr"],
        )
...
def linear_part(aff):
    ...
    return np.array([[aff.a, aff.b], [aff.d, aff.e]])


def shift_part(aff):
    """The translation vector associated with an affine transformation."""
    return np.array([aff.c, aff.f])


def combine_parts(linear, shift):
    """Put an affine transformation back together from the `linear` and the `shift` parts."""
    return Affine(
        linear[0, 0], linear[0, 1], shift[0], linear[1, 0], linear[1, 1], shift[1]
    )


def combine_affine(left_affine, right_affine):
    """Given two affine descriptions of image grids, create a grid
    that encompasses both images.
    """
    translation = shift_part(right_affine) - shift_part(left_affine)
    steps = np.linalg.solve(linear_part(left_affine), translation)
    shift = np.array(left_affine * np.where(steps > 0, 0, steps))
    return combine_parts(linear_part(left_affine), shift)
...
def merge_datasets(left_product, right_product, target_product, band_name):
    ...
    left_affine = Affine.from_gdal(*left_ds.attrs["geotransform"])
    right_affine = Affine.from_gdal(*right_ds.attrs["geotransform"])
    assert np.allclose(linear_part(left_affine), linear_part(right_affine))

    target_affine = combine_affine(left_affine, right_affine)

    left_end = ~target_affine * (left_affine * left_ds.shape)
    right_end = ~target_affine * (right_affine * right_ds.shape)
    target_shape = (
        int(max(left_end[0], right_end[0])),
        int(max(left_end[1], right_end[1])),
    )

    target_ds = target_product.create_dataset(
        band_name, shape=target_shape, dtype="int32", chunks=(1024, 1024)
    )
    target_ds[:] = 0
    target_count = target_product.create_dataset(
    ...
    def add_image(src_affine, src_product):
        """Add this image to the target array."""
        src_ds = find_dataset_by_name(src_product, band_name)

        # find the target window
        ul = ~target_affine * (src_affine * (0, 0))
        lr = ~target_affine * (src_affine * src_ds.shape)

        for coord in ul:
            assert abs(coord - np.around(coord)) < src_affine.precision

        for coord in lr:
            assert abs(coord - np.around(coord)) < src_affine.precision

        ul = [int(np.around(coord)) for coord in ul]
        lr = [int(np.around(coord)) for coord in lr]

        src_data, count_data = sum_and_count(src_product, band_name)

        # add the data to appropriate window in the target array
        target_ds[ul[0] : lr[0], ul[1] : lr[1]] += src_data
        target_count[ul[0] : lr[0], ul[1] : lr[1]] += count_data

    add_image(left_affine, left_product)
    add_image(right_affine, right_product)
    ...
    target_ds.attrs["geotransform"] = target_affine.to_gdal()
    target_ds.attrs["alias"] = band_name
    target_count.attrs["geotransform"] = target_affine.to_gdal()
    target_count.attrs["alias"] = band_name + "_pixelcount"
    ...
    for product in products:
        ...
        for band in band_names:
            if find_dataset_by_name(left_group[product], band) is None:
                ...
                merge_datasets(
                    left_group[product], right_group[product], target_product, band
                )
...
def merge_images(left, right, target):
    ...
    """
    with h5py.File(target) as target_fid, h5py.File(left, "r") as left_fid, h5py.File(
        right, "r"
    ) as right_fid:
        ...
        merge_groups(
            reflectance(left_granule), reflectance(right_granule), target_group
        )
