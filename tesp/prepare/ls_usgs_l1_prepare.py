...
def get_geo_ref_points(info):
    ...
    return {
        "ul": {
            "x": info["corner_ul_projection_x_product"],
            "y": info["corner_ul_projection_y_product"],
        },
        "ur": {
            "x": info["corner_ur_projection_x_product"],
            "y": info["corner_ur_projection_y_product"],
        },
        "ll": {
            "x": info["corner_ll_projection_x_product"],
            "y": info["corner_ll_projection_y_product"],
        },
        "lr": {
            "x": info["corner_lr_projection_x_product"],
            "y": info["corner_lr_projection_y_product"],
        },
    }
...
def get_mtl_content(acquisition_path):
    ...
    if acquisition_path.is_file() and tarfile.is_tarfile(str(acquisition_path)):
        with tarfile.open(str(acquisition_path), "r") as tp:
            try:
                internal_file = next(
                    filter(lambda memb: "MTL" in memb.name, tp.getmembers())
                )
...
def prepare_dataset(path):
    ...
    return {
        ...
        "image": {
            "bands": {
                band_name: {
                    "path": mtl_doc["product_metadata"][
                        "file_name_band_" + band_fname.lower()
                    ],
                    "layer": 1,
                }
...
def resolve_doc_offset(dataset_path, offset):
    ...
    if ".tar" in dataset_path.suffixes:
        return f"tar:{dataset_path}!{offset}"
    elif dataset_path.suffix == ".txt":
        return str(dataset_path.parent.absolute() / offset)
    else:
        raise NotImplementedError(f"Unexpected dataset path structure {dataset_path}")
...
def main(output, datasets, check_checksum, force_absolute_paths, newer_than):
    ...
    for ds in datasets:
        ...
        output_yaml = output / f"{_dataset_name(ds_path)}.yaml"
        ...
        prepare_and_write(
            ds_path, mtl_path, output_yaml, use_absolute_paths=force_absolute_paths
        )
...
def prepare_and_write(ds_path, mtl_path, output_yaml_path, use_absolute_paths=False):
...
def _dataset_name(ds_path):
    ...
    if ".tar" in ds_path.suffixes:
        ...
        raise NotImplementedError(f"Unexpected path pattern {ds_path}")
