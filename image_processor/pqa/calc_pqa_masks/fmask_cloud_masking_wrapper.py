...
def process(subprocess_list=[], resume=False):
    ...
    def FMaskCloudMask(mtl, null_mask=None, cloud_prob=None, wclr_max=None):
        ...
        try:  # Need to change directory to ensure log files are written to the right place
            ...
            (_, _, _, _, _, _, _, _, fmask_byte, _, _, _, _, _) = _fmask.plcloud_1_6sav(
                filename=mtl, cldprob=cloud_prob, mask=null_mask, wclr_max=wclr_max
            )
        finally:
    ...
    mtl = glob(os.path.join(l1t_input_dataset.pathname, "scene01/*_MTL.txt"))[
        0
    ]  # Crude but effective
