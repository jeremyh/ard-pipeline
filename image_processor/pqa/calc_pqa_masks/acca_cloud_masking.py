...
from ULA3.utils import dump_array, log_multiline
...
def process(subprocess_list=[], resume=False):
    ...
    assert (
        nbar_input_dataset
    ), "Unable to retrieve SceneDataset object for NBAR input scene dataset"
    ...
    # ===========================================================================
    assert (
        CONFIG.pqa_test_index["CONTIGUITY"] in result.test_set
    ), "Contiguity test not yet run"
    ...
    def CloudMask(image_stack, kelvin_array, contiguity_mask=None):
        ...
        if len(image_stack) == 0:
            return None
        assert type(image_stack[0]) == np.ndarray, "Image input is not valid"
        ...
        def water_test(reflectance_stack):
            ...
            return numexpr.evaluate(
                "(NDVI < 0.1) & (band5_array < 0.05)",
                {"NDVI": ndvi(reflectance_stack), "band5_array": reflectance_stack[4]},
                locals(),
            )
        ...
        def ndvi(reflectance_stack):
            ...
            return numexpr.evaluate(
                "(a - b) / (a + b)",
                {"a": reflectance_stack[3], "b": reflectance_stack[2]},
                locals(),
            )
        ...
        def ndsi(reflectance_stack):
            ...
            return numexpr.evaluate(
                "(a - b) / (a + b)",
                {"a": reflectance_stack[1], "b": reflectance_stack[4]},
                locals(),
            )
        ...
        def filter4(reflectance_stack):
            ...
            return numexpr.evaluate(
                "(1.0 - swir1) * TM6",
                {"swir1": reflectance_stack[4], "TM6": thermal_array},
                locals(),
            )
        ...
        def filter5(reflectance_stack):
            ...
            return numexpr.evaluate(
                "nir / red",
                {"nir": reflectance_stack[3], "red": reflectance_stack[2]},
                locals(),
            )
        ...
        def filter7(reflectance_stack):
            ...
            return numexpr.evaluate(
                "nir / swir1",
                {"nir": reflectance_stack[3], "swir1": reflectance_stack[4]},
                locals(),
            )
        ...
        def acca_2nd_pass(cloud_mask, ambiguous_array, thermal_array, mean_cloud_temp):
            ...
            upper = np.percentile(thermal_array[cloud_mask], 97.5)
            lower = np.percentile(thermal_array[cloud_mask], 83.5)
            upper_max = np.percentile(thermal_array[cloud_mask], 98.75)
            ...
            skew = skewness(
                thermal_array[cloud_mask],
                mean_temp=mean_cloud_temp,
                stdv_temp=cloud_stddev,
                count=cloud_count,
            )
        ...
        def acca(reflectance_stack, thermal_array, potential_cloud_array):
            ...
            ambiguous_array = (
                np.ones(potential_cloud_array.shape, dtype=np.float32) * NaN
            )
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter1.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array1.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f2": CONFIG.pqa_param["acca_thresh_f2"], "NaN": NaN},
                locals(),
            )
            ...
            find = numexpr.evaluate(
                "where(ndsi_array >= thresh_f2, 1, 0)",
                {"thresh_f2": CONFIG.pqa_param["acca_thresh_f2"]},
                locals(),
            )
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter2.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array2.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f3": CONFIG.pqa_param["acca_thresh_f3"], "NaN": NaN},
                locals(),
            )
            potential_cloud_array *= query
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter3.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array3.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f4": CONFIG.pqa_param["acca_thresh_f4"], "NaN": NaN},
                locals(),
            )

            # Get ambiguous pixels
            find = numexpr.evaluate(
                "_temporary >= thresh_f4",
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter4.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array4.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f5": CONFIG.pqa_param["acca_thresh_f5"], "NaN": NaN},
                locals(),
            )

            # Get ambiguous pixels
            find = numexpr.evaluate(
                "_temporary >= thresh_f5",
                {"thresh_f5": CONFIG.pqa_param["acca_thresh_f5"]},
                locals(),
            )
            ambiguous_array[find] = 1
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter5.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array5.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f6": CONFIG.pqa_param["acca_thresh_f6"], "NaN": NaN},
                locals(),
            )
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter6.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array6.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            query = numexpr.evaluate(
                ...
                {"thresh_f7": CONFIG.pqa_param["acca_thresh_f7"], "NaN": NaN},
                locals(),
            )

            # Tally filter 7 survivors
            f7_surv = np.nansum(query)
            ...
            # Get ambiguous pixels
            find = numexpr.evaluate(
                "_temporary <= thresh_f7",
                {"thresh_f7": CONFIG.pqa_param["acca_thresh_f7"]},
                locals(),
            )
            ambiguous_array[find] = 1
            ...
            if CONFIG.debug:
                dump_array(
                    query,
                    os.path.join(CONFIG.work_path, "acca_filter7.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
                dump_array(
                    potential_cloud_array,
                    os.path.join(CONFIG.work_path, "potential_cloud_array7.tif"),
                    template_dataset=nbar_input_dataset,
                    dtype=gdal.GDT_Float32,
                )
            ...
            cold_cloud = numexpr.evaluate(
                "_temporary < thresh_f8",
                {"thresh_f8": CONFIG.pqa_param["acca_thresh_f8"]},
                locals(),
            )
            ...
            cold_cloud_mean = np.mean(thermal_array[cold_cloud], dtype="float64")
            warm_cloud_pop = (float(warm_cloud.sum()) / ambiguous_array.size) * 100
            warm_cloud_mean = np.mean(thermal_array[warm_cloud], dtype="float64")
            ...
            if (
                Desert_Index <= CONFIG.pqa_param["acca_desertindex"]
                and snow_percent > CONFIG.pqa_param["acca_snow_threshold"]
            ):
                cloud = cold_cloud
            ...
            if cloud.sum() > 0:
                ...
                if (
                    (cold_cloud_pop > CONFIG.pqa_param["acca_coldcloud_pop"])
                    and (Desert_Index > CONFIG.pqa_param["acca_desertindex"])
                    and (
                    ...
                    if r_cloud is None:
                        ...
                        return r_cloud
                elif (Desert_Index <= CONFIG.pqa_param["acca_desertindex"]) and (
                    np.mean(thermal_array[cloud], dtype="float")
                    < CONFIG.pqa_param["acca_coldcloud_mean"]
                ):
                    return cold_cloud
        ...
        if CONFIG.debug:
            for band_index in range(dim[0]):
                reflectance_stack[band_index].dump(
                    os.path.join(
                        CONFIG.work_path, "reflectance_stack_" + str(band_index)
                    )
                )
        ...
        if CONFIG.debug:
            dump_array(
                thermal_array,
                os.path.join(CONFIG.work_path, "thermal_array.tif"),
                template_dataset=nbar_input_dataset,
                dtype=gdal.GDT_Float32,
            )
