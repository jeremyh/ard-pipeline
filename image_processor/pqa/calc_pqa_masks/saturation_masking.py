...
def process(subprocess_list=[], resume=False):
    ...
    def SaturationMask(band_array, under_sat=1, over_sat=255, use_numexpr=True):
        ...
        if use_numexpr:
            sublogger.debug(
                'numexpr used: numexpr.evaluate("(band_array != %i) & (band_array != %i)")'
                % (under_sat, over_sat)
            )
            mask = numexpr.evaluate(
                "(band_array != under_sat) & (band_array != over_sat)"
            )
        else:
            sublogger.debug(
                "numpy used: (band_array != %i) & (band_array != %i)"
                % (under_sat, over_sat)
            )
    ...
    pq_const.getArrayBandLookup(band_list)
