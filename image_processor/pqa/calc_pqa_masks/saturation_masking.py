...
def process(subprocess_list=[], resume=False):
    ...
    def SaturationMask(band_array, use_numexpr=True):
        ...
        if use_numexpr:
            sublogger.debug(
                'numexpr used: numexpr.evaluate("band_array == 1 or band_array == 255")'
            )
    ...
    assert (
        len(full_band_list) == l1t_stack.shape[0]
    ), "Mismatch between array depth and band count"
    ...
    bit_index_list = [
        CONFIG.pqa_test_index["SATURATION"][band_file_no]
        for band_file_no in sorted(
            set(l1t_input_dataset.satellite.BAND_TYPES["REFLECTIVE"])
            | set(l1t_input_dataset.satellite.BAND_TYPES["THERMAL"])
        )
    ]
    assert (
        len(bit_index_list) == l1t_stack.shape[0]
    ), "Unable to determine bit indices for all bands"
    ...
    for band in band_list:
        ...
        sublogger.debug(
            "Processing saturation for band = %d, band_index = %d, bit_index = %d",
            band,
            band_index,
            bit_index_list[band_index],
        )
        ...
        if (
            bit_index == 5 and 6 not in bit_index_list
        ):  # If this is thermal band 1 and there is no thermal band 2 in this dataset
            bit_index = 6
