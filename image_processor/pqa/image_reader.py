...
def process(subprocess_list=[], resume=False):
    ...
    assert l1t_stack.shape[0] == len(
        l1t_input_dataset.satellite.BAND_TYPES["REFLECTIVE"]
    ) + len(
        l1t_input_dataset.satellite.BAND_TYPES["THERMAL"]
    ), "Unexpected number of L1T bands"

    nbar_stack = nbar_input_dataset.ReadAsArray()
    assert nbar_stack.shape[0] == len(
        l1t_input_dataset.satellite.BAND_TYPES["REFLECTIVE"]
    ), "Unexpected number of nbar bands"
