...
def process(subprocess_list=[], resume=False):
    ...
    write_metadata(
        DATA,
        CONFIG,
        os.path.join(os.path.dirname(__file__), CONFIG.PQA_XML_METADATA_TEMPLATE),
        CONFIG.PQA_PROCESSOR_VERSION,
    )
...
def write_metadata(DATA, CONFIG, xml_metadata_template, PROCESSOR_VERSION):
    ...
    def compose_lineage(l1t_input_dataset, tests_run):
        ...
        """
        run_string = {"0": "Not Run", "1": "Run"}

        test_const_list = list(
            CONFIG.pqa_test_consts
        )  # Ordered list of all test constants as defined in processor.conf
...
"""
    ...
    pqa_tif_path = find_files(os.path.join(pqa_temp_output, "scene01"), r"L.*\.tif")[-1]
    ...
    pqa_dataset.set_metadata_object(
        new_metadata
    ...
    #                           Empty Test: Not Set"""
    # ===========================================================================
    pqa_dataset.lineage_statement = compose_lineage(
        l1t_input_dataset, pqa_log_info.tests_run
    )
...
""".format(
    ...
    root_tif_filename = find_files(os.path.join(pqa_temp_output, "scene01"), "L.*.tif")[
        -1
    ...
    log_multiline(
        logger.info, new_metadata._metadata_dict, "Metadata to be output for PQA" "\t"
    )
