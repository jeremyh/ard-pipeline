...
class PQALogExtractor:
    def __init__(self, pqa_tif_path):
        logfile_list = [
            os.path.join(os.path.dirname(pqa_tif_path), logfile)
            for logfile in [
                "ACCA_LOGFILE.txt",
                "FMASK_LOGFILE.txt",
        ...
        self.tests_run = list(
            (os.path.splitext(os.path.basename(pqa_tif_path))[0]).split("_")[-1]
        )
