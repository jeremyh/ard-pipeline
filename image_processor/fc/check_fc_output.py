...
def process(subprocess_list=[], resume=False):
    ...
    def backup_output_path(fc_output_path, work_path):
        ...
        if result["stdout"]:
            log_multiline(
                logger.info,
                result["stdout"],
                command_string + " in " + fc_temp_output,
                "\t",
            )
    ...
    def check_dataset_id():
        ...
        def get_fc_dataset_id():
            ...
            assert station_code, (
                "Unrecognised ground station ID %s" % nbar_input_dataset.ground_station
            )
        ...
        DATA.set_item(
            "fc_temp_output.dat", fc_temp_output
        )  # SMR SEP14: side effect here

        # Default output_path to standard directory under FC_DATA_ROOT if not specified
        fc_output_path = os.path.abspath(
            CONFIG.output_path or os.path.join(CONFIG.output_root, fc_dataset_id)
        )
        logger.info("fc_output_path: %s", fc_output_path)
        DATA.set_item(
            "fc_output_path.dat", fc_output_path
        )  # SMR SEP14: side effect here

        old_dataset_id = re.match(
            ".*(?=_)", os.path.basename(nbar_input_dataset.root_dataset_pathname)
        ).group()
        old_fc_tif_template = os.path.join(
            fc_output_path, "scene01", "%s_*.tif" % old_dataset_id
        )
        final_fc_tif_template = os.path.join(
            fc_output_path, "scene01", "%s_*.tif" % fc_dataset_id
        )
        logger.debug(
            "old_dataset_id = %s, old_fc_tif_template = %s, final_fc_tif_template = %s",
            old_dataset_id,
            old_fc_tif_template,
            final_fc_tif_template,
        )

        final_fc_tif_filename = None
        filelist = glob(old_fc_tif_template) or glob(
            re.sub(".tif$", ".TIF$", old_fc_tif_template)
        )
        if filelist:  # Old format FC output filename found
            old_fc_tif_filename = filelist[0]  # There should be only one
            m = re.match(
                r"(.*/)(.*)(_\d+\.\w+)", old_fc_tif_filename
            ...
            filelist = glob(final_fc_tif_template) or glob(
                re.sub(".tif$", ".TIF$", final_fc_tif_template)
            )
            if filelist:
        ...
        if final_fc_tif_filename:  # Final output already exists
            ...
            logger.debug(
                ...
                nbar_input_dataset.completion_datetime,
            )
            input_datetime = (
                nbar_input_dataset.completion_datetime
            ...
            output_datetime = output_datetime or datetime.fromtimestamp(
                os.path.getmtime(final_fc_tif_filename)
            )
            ...
            if output_datetime > input_datetime:
                logger.debug(
                    "%s is newer than %s"
                    % (final_fc_tif_filename, nbar_input_dataset.root_dataset_pathname)
                )
                ...
                if CONFIG.repackage:  # Repackaging required
                    logger.debug(
                        ...
                        nbar_input_dataset.root_dataset_pathname,
                    )
        ...
        try:
            ...
            logger.info(
                "Created temporary FC output directory %s with sub-directory scene01",
                fc_temp_output,
            )
        except OSError, e:
