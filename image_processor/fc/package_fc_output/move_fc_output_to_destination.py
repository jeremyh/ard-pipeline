...
def process(subprocess_list=[], resume=False):
    ...
    if (
    ...
    ):  # Moving is actually required (i.e. temp directory used)
        # N.B: Move files rather than copying them - may change later
        command_string = "rm -rf {}; mkdir -p {}; mv -f {}/* {}".format(
            fc_output_path, fc_output_path, fc_temp_output, fc_output_path
        )
        ...
        logger.info(
            "FC output already in final destination directory %s. No files moved",
            fc_output_path,
        )
