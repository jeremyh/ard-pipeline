...
def process(subprocess_list=[], resume=False):
    ...
    for _level in CONFIG.input.keys():
        ...
        if not re.search(input_re_pattern, CONFIG.input[_level]["name"]):
            logger.warning(
                "Non-standard input pathname: %s", CONFIG.input[_level]["path"]
            )
        ...
        log_multiline(
            logger.info,
            _input_dataset.__dict__,
            "SceneDataset object for %s" % _input_dataset._pathname,
            "\t",
        )
        if CONFIG.debug:
            log_multiline(
                logger.info,
                _input_dataset._metadata._metadata_dict,
                "Full metadata %s" % _input_dataset._pathname,
                "\t",
            )
            log_multiline(
                logger.info,
                _input_dataset.satellite.__dict__,
                "Satellite object for %s" % _input_dataset._pathname,
                "\t",
            )

        # Check whether processing level is valid
        logger.info(
            "Input dataset processor level = %s", _input_dataset.processor_level
        )
        if _level == "l1t":
            assert _input_dataset.processor_level.lower() in ["l1t", "ortho"], (
                "Dataset is " + _input_dataset.processor_level
            )
        else:
            assert _input_dataset.processor_level.lower() == _level, (
                "Dataset is " + _input_dataset.processor_level
            )
