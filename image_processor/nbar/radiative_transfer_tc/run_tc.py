...
def process(subprocess_list=[], resume=False):
    ...
    if dump_path:
        ...
        l1_shape.write_header_slope_file(
            os.path.join(dump_path, "SLOPE_ANGLE_INPUTS"), pixel_buf
        )

    # solar angle data (these are set in ULA3.image_processor.utils.calc_solar_grids).
    solar_angle = as_array(
        DATA.get_item("SOL_Z_DEG.bin", DataGrid).array, dtype=numpy.float32
    )
    sazi_angle = as_array(
        DATA.get_item("SOL_AZ_DEG.bin", DataGrid).array, dtype=numpy.float32
    )

    # satellite angle data (these are set in ULA3.image_processor.utils.calc_satellite_grids)
    view_angle = as_array(
        DATA.get_item("SAT_V_DEG.bin", DataGrid).array, dtype=numpy.float32
    )
    azi_angle = as_array(
        DATA.get_item("SAT_AZ_DEG.bin", DataGrid).array, dtype=numpy.float32
    )
    rela_angle = as_array(
        DATA.get_item("REL_AZ_DEG.bin", DataGrid).array, dtype=numpy.float32
    )
    ...
    if dump_path:
        write_tif_file(
            l1t_input_dataset,
            solar_angle,
            os.path.join(dump_path, "solar_angle.img"),
            file_type="ENVI",
        )
        write_tif_file(
            l1t_input_dataset,
            sazi_angle,
            os.path.join(dump_path, "sazi_angle.img"),
            file_type="ENVI",
        )
        write_tif_file(
            l1t_input_dataset,
            rela_angle,
            os.path.join(dump_path, "rela_angle.img"),
            file_type="ENVI",
        )
        write_tif_file(
            l1t_input_dataset,
            view_angle,
            os.path.join(dump_path, "view_angle.img"),
            file_type="ENVI",
        )
        write_tif_file(
            l1t_input_dataset,
            azi_angle,
            os.path.join(dump_path, "azi_angle.img"),
            file_type="ENVI",
        )
    ...
    if dump_path:
        write_tif_file(
            l1t_input_dataset,
            shadow_s,
            os.path.join(dump_path, "shadow_s.img"),
            file_type="ENVI",
        )
        write_tif_file(
            l1t_input_dataset,
            shadow_v,
            os.path.join(dump_path, "shadow_v.img"),
            file_type="ENVI",
        )
    ...
    for band_number in l1t_input_dataset.bands("REFLECTIVE"):
        ...
        param_file = open(
            os.path.join(CONFIG.work_path, "brdf_modis_band%i.txt" % band_number), "r"
        )
        ...
        ref_lm, ref_brdf, ref_terrain = run_brdfterrain(
            ...
            slope_results.rela_slope,
            load_bin_file(
                boo[(band_number, "a")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "b")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "s")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "fs")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "fv")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "ts")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "dir")], region_nrow, region_ncol, dtype=numpy.float32
            ),
            load_bin_file(
                boo[(band_number, "dif")], region_nrow, region_ncol, dtype=numpy.float32
            ),
        )
        ...
        write_tif_file(
            ...
            ref_lm,
            os.path.join(
                work_path, pref + "ref_lm_" + str(band_number) + output_extension
            ),
        ...
        write_tif_file(
            ...
            ref_brdf,
            os.path.join(
                work_path, pref + "ref_brdf_" + str(band_number) + output_extension
            ),
            file_type=output_format,
        )
        write_tif_file(
            ...
            ref_terrain,
            os.path.join(
                work_path, pref + "ref_terrain_" + str(band_number) + output_extension
            ),
            file_type=output_format,
        )

        outfname = os.path.join(
            output_path, "scene01", "%s_B%d%s" % (nbar_dataset_id, band_number, ".tif")
        )
