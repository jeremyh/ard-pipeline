...
from ULA3.modtran import modtran_config, run_modtran
...
def process(subprocess_list=[], resume=False):
    ...
    _FULL_SUBPROCESS_LIST = [
        coord + "." + albedo
        for coord in modtran_config.FULL_COORD_LIST
        for albedo in modtran_config.FULL_ALBEDO_LIST
    ]
    ...
    for subprocess in _subprocess_list:
        if subprocess:
            ...
            assert (
                len(module_list) <= 2
            ), "Only coordinator and/or albedo can be specified"
            if len(module_list) == 1:  # Only one sub-module specified
                if (
                    module in modtran_config.FULL_COORD_LIST
                ):  # Coordinator only specified
                    _subsubprocess_list = [
                        [module, albedo] for albedo in modtran_config.FULL_ALBEDO_LIST
                    ]
                elif module in modtran_config.FULL_ALBEDO_LIST:  # Albedo only specified
                    _subsubprocess_list = [
                        [coord, module] for coord in modtran_config.FULL_COORD_LIST
                    ]
                else:
                    raise Exception(
                        "run_modtran error: Invalid coordinator or albedo specified: %s"
                        % repr(module)
                    )
                ...
                if (
                    module_list[1] in modtran_config.FULL_COORD_LIST
                    and module_list[0] in modtran_config.FULL_ALBEDO_LIST
                ):
                    module_list = (module_list[1], module_list[0])
                assert (
                    module_list[0] in modtran_config.FULL_COORD_LIST
                    and module_list[1] in modtran_config.FULL_ALBEDO_LIST
                ), "Invalid coordinator or albedo specified"
