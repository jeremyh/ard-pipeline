...
from ULA3.modtran import modtran_config, run_bilinear_ortho
...
def process(subprocess_list=[], resume=False):
    ...
    full_subprocess_list = [
        [band_number, factor]
        for band_number in band_string_list
        for factor in modtran_config.FULL_FACTOR_LIST
    ]
        ...
        if subprocess:
            ...
            if len(module_list) == 1:  # Only one sub-module specified
                ...
                if module in band_string_list:  # band_index only specified
                    _expanded_subprocess_list += [
                        [module, factor] for factor in modtran_config.FULL_FACTOR_LIST
                    ]
                elif module in modtran_config.FULL_FACTOR_LIST:  # Factor only specified
                    _expanded_subprocess_list += [
                        [band_number, module] for band_number in band_string_list
                    ]
                else:
                    sublogger.warning(
                ...
                if (
                    module_list[0] in modtran_config.FULL_FACTOR_LIST
                    and module_list[1] in band_string_list
                ):
                    module_list = (module_list[1], module_list[0])
                else:
                    if not (
                        module_list[0] in band_string_list
                        and module_list[1] in modtran_config.FULL_FACTOR_LIST
                    ):
                        sublogger.warning(
                            "Ignoring invalid band number or factor: %s", module_list
                        )
    ...
    with _output_lock:
        ...
        if outputs is None:
            outputs = DATA.save_item(
                item_path="bilinear_ortho_outputs", item_instance={}
            )

    for subprocess in _expanded_subprocess_list:
        band_number = int(subprocess[0])
        factor = subprocess[1]
        outputs[(band_number, factor)] = run_bilinear_ortho(
            band_number, factor, sublogger, CONFIG.work_path, CONFIG.BIN_DIR
        )
