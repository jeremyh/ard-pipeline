...
from ULA3.ancillary.brdf import (
    average_brdf_value,
    get_brdf_dirs_modis,
    get_brdf_dirs_pre_modis,
)
...
def get_average_brdf_value(
    ...
    assert band_number > 0 and band_number <= l1t_input_dataset.RasterCount, (
        "Invalid band number %d" % band_number
    )

    wavelength_range = l1t_input_dataset.sensor_band_info(band_number)["WAVELENGTH"]
    assert wavelength_range, (
        "No info for band index %d found in configuration" % band_number
    )
...
def process(subprocess_list=[], resume=False):
    ...
    band_strings = [
        str(band_number) for band_number in l1t_input_dataset.bands("REFLECTIVE")
    ]

    # Full list of subprocesses to be run if none are specified
    full_subprocess_list = [
        [band_string, factor] for band_string in band_strings for factor in _FACTOR_LIST
    ]
    ...
    for subprocess in _subprocess_list:
        if subprocess:
            ...
            assert (
                len(module_list) <= 2
            ), "Only band_index and/or factor can be specified"
            if len(module_list) == 1:  # Only one sub-module specified
                module = module_list[0]
                if module in band_strings:  # band_index only specified
                    _expanded_subprocess_list += [
                        [module, factor] for factor in _FACTOR_LIST
                    ]
                elif module in _FACTOR_LIST:  # Factor only specified
                    _expanded_subprocess_list += [
                        [band_number, module] for band_number in band_strings
                    ]
                else:
                    sublogger.warning(
                        "Ignoring invalid band number or factor: %s", module_list
                    )
                    continue
                ...
                if module_list[0] in _FACTOR_LIST and module_list[1] in band_strings:
                    ...
                    if not (
                        module_list[0] in band_strings
                        and module_list[1] in _FACTOR_LIST
                    ):
                        sublogger.warning(
                            "Ignoring invalid band number or factor: %s", module_list
                        )
    ...
    _expanded_subprocess_list = [
        _expanded_subprocess_list[i]
        for i, x in enumerate(_expanded_subprocess_list)
        if x not in _expanded_subprocess_list[i + 1 :]
    ]
    ...
    modis_brdf_range = [
        datetime.date(*[int(x) for x in y.split(".")]) for y in modis_brdf_dir_range
    ]

    use_jupp_li_brdf = (
        l1t_input_dataset.scene_centre_date < modis_brdf_range[0]
        or l1t_input_dataset.scene_centre_date > modis_brdf_range[1]
    )
    ...
    assert os.path.exists(brdf_root), (
        "brdf_root [%s] not found. Aborting NBAR..." % brdf_root
    )
    ...
    assert (
        success
    ), "Primary and secondary BRDF extraction both failed. Aborting NBAR calculation"
