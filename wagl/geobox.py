...
class GriddedGeoBox:
    ...
    def project_extents(self, to_crs):
        ...
        column_ids = np.arange(column_ur_idx)
        row_ids = np.arange(row_ll_idx)

        # index coords of every pixel along geobox extents
        top = (column_ids, np.array([0] * column_ur_idx))
        right = (np.array([column_ur_idx] * row_ll_idx), row_ids)
        bottom = (column_ids[::-1], np.array([row_ll_idx] * column_ur_idx))
        left = (np.array([0] * row_ll_idx), row_ids[::-1])
