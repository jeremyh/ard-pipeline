...
class BrdfTileSummary:
    ...
    def __add__(self, other):
        ...
        return BrdfTileSummary(
            ...
            {key: self.brdf_sums[key] + other.brdf_sums[key] for key in BrdfParameters},
    ...
    def mean(self):
        ...
        if self.valid_pixels == 0 or self.brdf_sums[BrdfParameters.ISO] == 0.0:
            ...
            return {
                key: dict(value=(1 if key == BrdfParameters.ISO else 0))
...
def load_brdf_tile(src_poly, src_crs, ds, uri):
    ...
    def layer_sum(i):
        ...
        layer = float(ds.attrs["scale_factor"]) * (
            layer - float(ds.attrs["add_offset"])
        )
        return np.nansum(layer)

    return BrdfTileSummary(
        valid_pixels,
        {key: layer_sum(index) for index, key in enumerate(BrdfParameters)},
        [uri],
    )
...
def get_brdf_data(
    ...
    for tile in tile_list:
        with h5py.File(tile, "r") as fid:
            tally += load_brdf_tile(
                src_poly, src_crs, fid[acquisition.brdf_dataset], tile
            )
