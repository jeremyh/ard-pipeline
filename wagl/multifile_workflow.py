...
from wagl.hdf5 import H5CompressionFilter, create_external_link
...
class WriteTp5(luigi.Task):
    ...
    def requires(self):
        ...
        tasks["ancillary"] = AncillaryData(
            self.level1, self.work_root, self.granule, self.vertices, self.workflow
        )
...
class Atmospherics(luigi.Task):
    ...
    def run(self):
        nvertices = self.vertices[0] * self.vertices[1]
        with self.output().temporary_path() as out_fname:
            link_atmospheric_results(self.input(), out_fname, nvertices, self.workflow)
...
class CalculateCoefficients(luigi.Task):
    ...
    def run(self):
        with self.output().temporary_path() as out_fname:
            _calculate_coefficients(
                self.input().path, out_fname, self.compression, self.filter_opts
            )
...
class InterpolateCoefficient(luigi.Task):
    ...
    def requires(self):
        args = [self.level1, self.work_root, self.granule, self.vertices]
        return {
            "comp": CalculateCoefficients(*args, workflow=self.workflow),
    ...
    def output(self):
        ...
        out_fname = f"{self.coefficient.value}-{self.band_name}.h5"
        return luigi.LocalTarget(pjoin(out_path, out_fname))
...
class IncidentAngles(luigi.Task):
    ...
    def requires(self):
        ...
        return {
            ...
            "slp_asp": SlopeAndAspect(
                *args, dsm_fname=self.dsm_fname, buffer_distance=self.buffer_distance
            ),
        }
...
class SurfaceReflectance(luigi.Task):
    ...
    def output(self):
        out_path = pjoin(self.work_root, self.group, self.base_dir)
        fname = f"reflectance-{self.band_name}.h5"
...
class SurfaceTemperature(luigi.Task):
    ...
    def output(self):
        out_path = pjoin(self.work_root, self.group, self.base_dir)
        fname = f"temperature-{self.band_name}.h5"
        return luigi.LocalTarget(pjoin(out_path, fname))
...
class LinkwaglOutputs(luigi.Task):
    ...
    def output(self):
        out_fname = pjoin(dirname(self.work_root), f"{self.granule}.h5")
        return luigi.LocalTarget(out_fname)
...
class ARD(luigi.WrapperTask):
    ...
    def requires(self):
        ...
        for level1 in level1_list:
            ...
            work_name = f"{container.label}.wagl"
            for granule in container.granules:
