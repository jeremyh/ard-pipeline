...
class AcquisitionsContainerTest(unittest.TestCase):
    ...
    def test_granules_ls8_scene1(self):
        scene = acquisitions(LS8_SCENE1)
        assert scene.granules[0] == "LC80900842013284LGN00"

    def test_granules_ls8_scene1C2(self):
        scene = acquisitions(LS8_SCENE1C2)
        assert scene.granules[0] == "LC80920842020303LGN00"

    def test_granules_ls8_sceneRTC2(self):
        scene = acquisitions(LS8_SCENERTC2)
        assert scene.granules[0] == "LC81060632021051LGN00"
...
class Landsat5Scene1AcquisitionTest(unittest.TestCase):
    ...
    def test_samples(self):
        assert self.acqs[0].samples == 74

    def test_lines(self):
        assert self.acqs[0].lines == 65

    def test_read(self):
        assert self.acqs[0].data()[40, 30] == 53

    def test_spectral_filter_cfg_vsir(self):
        assert self.acqs[0].spectral_filter_name == "landsat5_vsir.flt"

    def test_spectral_filter_cfg_thermal(self):
        assert self.acqs[5].spectral_filter_name == "landsat5_thermal.flt"
...
class Landsat8Mtl1AcquisitionTest(unittest.TestCase):
    ...
    def test_lines(self):
        assert self.acqs[0].lines == 75

    def test_read(self):
        assert self.acqs[0].data()[70, 30] == 0

    def test_spectral_filter_cfg_vsir(self):
        assert self.acqs[0].spectral_filter_name == "landsat8_vsir.flt"

    def test_spectral_filter_cfg_thermal(self):
        assert self.acqs[1].spectral_filter_name == "landsat8_thermal.flt"
...
class Landsat8Mtl1c2AcquisitionTest(unittest.TestCase):
    ...
    def test_acquisition_datetime(self):
        for acq in self.acqs:
            assert acq.acquisition_datetime == datetime.datetime(
                2020, 10, 29, 0, 2, 59, 26835
            )

    def test_min_radiance_band1(self):
        assert self.acqs[0].min_radiance == -63.61862

    def test_max_radiance_band1(self):
        assert self.acqs[0].max_radiance == 770.3847
    ...
    def test_solar_azimuth(self):
        assert self.acqs[0].solar_azimuth == 57.65543514

    def test_solar_elevation(self):
        assert self.acqs[0].solar_elevation == 56.77807119
...
class Landsat8Mtl1c2rtAcquisitionTest(unittest.TestCase):
    ...
    def test_acquisition_datetime(self):
        for acq in self.acqs:
            assert acq.acquisition_datetime == datetime.datetime(
                2021, 2, 20, 1, 20, 51, 238295
            )

    def test_min_radiance_band1(self):
        assert self.acqs[0].min_radiance == -64.20548

    def test_max_radiance_band1(self):
        assert self.acqs[0].max_radiance == 777.49127
    ...
    def test_solar_azimuth(self):
        assert self.acqs[0].solar_azimuth == 103.88291729

    def test_solar_elevation(self):
        assert self.acqs[0].solar_elevation == 58.66464407
