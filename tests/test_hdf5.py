...
class HDF5Test(unittest.TestCase):
    ...
    def test_blosc_zstandard_default(self):
        ...
        kwargs = (
            H5CompressionFilter.BLOSC_ZSTANDARD.config().dataset_compression_kwargs()
        )
    ...
    def test_write_h5_image_multiband(self):
        ...
        dtype = np.dtype([(bname, "int16") for bname in band_names])

        dataset = np.ndarray(shape=self.image_data.shape, dtype=dtype)
        ...
        with h5py.File(fname, "w", **self.memory_kwargs) as fid:
            ...
            for bname in band_names:
                test = fid["image"].attrs[f"{bname}_MINMAXRANGE"]
                assert (minmax == test).all()
    ...
    def test_dataframe_roundtrip(self):
        ...
        """
        df = pd.DataFrame(self.table_data)
        df["timestamps"] = pd.date_range("1/1/2000", periods=10, freq="D", tz="UTC")
        ...
        with h5py.File(fname, "w", **self.memory_kwargs) as fid:
            ...
            assert df.equals(hdf5.read_h5_table(fid, "dataframe"))
