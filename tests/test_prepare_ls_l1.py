...
def test_prepare_l1_tarball(tmpdir):
    ...
    assert doc["extent"]["coord"]["ll"] == approx(
        {"lat": -26.954354903739272, "lon": 129.22769548746237}
    )
    assert doc["extent"]["coord"]["lr"] == approx(
        {"lat": -26.928774471741676, "lon": 131.69588786021484}
    )
    assert doc["extent"]["coord"]["ul"] == approx(
        {"lat": -25.03371801797915, "lon": 129.22402339672468}
    )
    assert doc["extent"]["coord"]["ur"] == approx(
        {"lat": -25.01021635833341, "lon": 131.65247288941694}
    )
