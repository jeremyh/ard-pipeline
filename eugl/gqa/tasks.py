...
from wagl.constants import BandType, GroupName
...
class GQATask(luigi.Task):
    ...
    def run(self):
        ...
        try:
            ...
            if land:
                location = f"{self.granule}/{self.land_band}"
            else:
                location = f"{self.granule}/{self.ocean_band}"
            ...
            _LOG.debug(
                "granule %s found reference images %s",
                self.granule,
            ...
            if land:
                ...
                cmd = gverify_cmd(
                    self, vrt_file, source_band, temp_directory, extra=extra
                )
                ...
                collect_gcp(
                    self.gverify_root_fix_qa_location, landsat_scenes, points_txt
                )
    ...
    with open(points_txt, "w") as fl:
        for scene in landsat_scenes:
            ...
            with open(this_points_txt) as fl2:
                for l in fl2:
                    _LOG.debug(f"GQA: {this_points_txt} says {l}")
                    fl.write(l)
...
def parse_gqa(task, output_yaml, reference_images, band_id, sat_id, work_dir):
    ...
    _LOG.debug(f"ref_date for {first_ref} is {ref_date}")
    result = dict(
...
def gverify_cmd(
    task, reference, source, work_dir, extra=None, resampling=Resampling.bilinear
):
    ...
    wrapper = [
        f"export LD_LIBRARY_PATH={task.gverify_ld_library_path}:$LD_LIBRARY_PATH; ",
    ...
    gverify = [
        ...
        work_dir,
        "-p",
        str(task.gverify_pyramid_levels),
        "-n",
        str(task.gverify_thread_count),
        "-nv",
        str(task.gverify_null_value),
        "-c",
        str(task.gverify_correlation_coefficient),
        "-r",
        resampling_method[resampling],
        "-cs",
        str(task.gverify_chip_size),
    ]

    return ["bash", "-c", "'{}'".format(" ".join(wrapper + gverify + extra))]
...
def acquisition_timestamp(h5_file, granule):
    result = h5_file[f"{granule}/{GroupName.ATMOSPHERIC_INPUTS_GRP.value}"].attrs[
...
def find_reference_date(filename, band_id, sat_id):
    ...
    pattern2 = re.compile(
        "p(?P<path>[0-9]{3})r(?P<row>[0-9]{3})(?P<junk>_[A-Za-z, 0-9]{3})"
...
def get_reference_directory(scene_name, landsat_path, landsat_row):
    ...
    if not isdir(reference_dir):
        _LOG.info("No reference directory (%r) for %r", reference_dir, scene_name)
        _LOG.info("Trying backup reference directory: (%r)", bckup_ref_dir)
        ref_dir = bckup_ref_dir
        reference_dir = pjoin(ref_dir, path, row)
        if not isdir(reference_dir):
            _LOG.info(
                "No backup reference directory (%r) for %r", reference_dir, scene_name
            )
...
class RunGverify(luigi.Task):
    ...
    def on_failure(self, exception):
        ...
        report = [
            "Status: fail\n",
            f"gverify error code: {return_code}",
            f"scene_id: {scene_id}",
            f"reference_filename: {ref_fname}",
            f"reference_acquisition_date: {ref_date}",
            msg,
        ]
...
class CalculateGQA(luigi.Task):
    ...
    def run(self):
        ...
        results_fname = results_fmt.format(
            acquisition_date=src_date, reference_date=ref_date, band=band, fmt=fmt
        )
        results_fname = pjoin(out_path, results_fname)

        # calculate the gqa figures
        out_fname = pjoin(out_path, CONFIG.get("gqa", "gqa_output_format"))
        _LOG.info("Calculating results file %r -> %r", results_fname, out_fname)
        calculate_gqa(
            results_fname,
            out_fname,
            ref_fname,
