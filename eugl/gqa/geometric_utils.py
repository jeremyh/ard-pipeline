...
def call_gverify(
    ...
    wrapped_cmd = (
        ...
        (
            ...
            "{gverify_cmd}".format(
                lib_path=GVERIFY_LIB_PATH, gverify_cmd=" ".join(gverify_cmd)
            )
        ),
    )
...
def gverify(
    ...
    for fmt in gverify_output_format:
        ...
        out_fname = renamed_format.format(
            acquisition_date=src_date,
...
def calculate_gqa(
    ...
    ptrn = r"(?P<src_date>[0-9]{8})_(?P<ref_date>[0-9]{7,9})_(?P<stuff>\w*)"
    ...
    for i in range(iterations):
        ...
        subset = subset[
            (abs(subset.X_Residual - mean_x) < (stddev * stddev_x))
            & (abs(subset.Y_Residual - mean_y) < (stddev * stddev_y))
        ]
    ...
    abs_ = {
        _clean_name(i).split("_")[-1]: _rounded(
            tr[tr.Residual_XY == i].Residual.values[0]
        )
...
def reproject(source_fname, reference_fname, out_fname, resampling=Resampling.bilinear):
    """Reproject an image.
...
def get_reference_data(acquisition, base_reference_dir):
    ...
    path = f"{int(acquisition.path):0=3d}"
    row = f"{int(acquisition.row):0=3d}"
    ...
    if match is not None:
        ...
        for fname in ref_imgs:
            ...
            if ref_band == bnum:
                df = df.append(
                    {"ref_fname": fname, "date": ref_date}, ignore_index=True
                )
        ...
        for fname in ref_imgs:
            match = re.match(ptrn, fname)
            ref_band = match.group("band")
            ref_date = datetime.datetime.strptime(match.group("date"), "%Y%m%d")
