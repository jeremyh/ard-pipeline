...
class GetAncillaryData(luigi.Task):
    ...
    def output(self):
        out_path = acquisitions(self.level1).get_root(
            self.nbar_root, self.group, self.granule
        )
...
class RunModtranCase(luigi.Task):
    ...
    def output(self):
        out_path = acquisitions(self.level1).get_root(
            self.nbar_root, granule=self.granule
        )
...
class RunAccumulateSolarIrradianceCase(luigi.Task):
    ...
    def output(self):
        ...
        fname = f"{self.coord}-alb-{self.albedo}.h5"
    ...
    def run(self):
        ...
        with self.output().temporary_path() as out_fname:
            gaip._calculate_solar_radiation(
                flux_fname, response_fname, transmittance, out_fname, self.compression
            )
    ...
    def requires(self):
        for point in range(self.npoints):
            for albedo in self.albedos:
                args = [self.level1, self.nbar_root, self.granule, point, albedo]
...
class BilinearInterpolationBand(luigi.Task):
    ...
    def output(self):
        ...
        out_fname = f"{self.factor}-band-{self.band}.h5"
        return luigi.LocalTarget(pjoin(out_path, self.base_dir, out_fname))
...
class BilinearInterpolation(luigi.Task):
    ...
    factors = luigi.ListParameter(
        default=["fv", "fs", "b", "s", "a", "dir", "dif", "ts"], significant=False
    )
