...
class CalculateCastShadow(luigi.Task):
    ...
    def requires(self):
        return [
            CalculateCastShadowSun(self.l1t_path),
...
class TerrainCorrection(luigi.Task):
    ...
    def requires(self):
        return [
            BilinearInterpolation(self.l1t_path),
            DEMExctraction(self.l1t_path),
    ...
    def output(self):
        ...
        for level in rfl_levels:
            for band in bands_to_process:
                target = pjoin(outdir, output_format.format(level=level, band=band))
                targets.append(luigi.LocalTarget(target))
        return targets
    ...
    def run(self):
        ...
        for level in rfl_levels:
            for band in bands_to_process:
                rfl_lvl_fnames[(band, level)] = pjoin(
                    outdir, output_format.format(level=level, band=band)
                )
