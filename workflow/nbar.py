...
class CalculateLonGrid(luigi.Task):
    ...
    def run(self):
        ...
        out_path = container.get_root(self.out_path, group=group, granule=granule)
...
class WriteTp5(luigi.Task):
    ...
    def run(self):
        ...
        # load an acquisition
        # TODO: we haven't passed in a granule
        acq = container.get_acquisitions(group=self.group, granule=self.granule)[0]
...
class PrepareModtranInput(luigi.Task):
    ...
    def requires(self):
        ...
        if bool(int(CONFIG.get("fuqin_requires", "required"))):
            tasks = [FilesRequiredByFugin(*args), WriteTp5(*args)]
        else:
            tasks = [WriteTp5(*args)]
    ...
    def requires(self):
        ...
        for coord in coords:
            for albedo in albedos:
                reqs.append(
                    RunModtranCase(
                        self.l1t_path, self.out_path, self.granule, coord, albedo
                    )
                )
        return reqs
