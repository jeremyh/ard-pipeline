...
class GetBrdfAncillaryData(luigi.Task):
    ...
    def run(self):
        ...
        value = gaip.get_brdf_data(acqs[0], brdf_path, brdf_premodis_path, out_path)
        save(self.output(), value)
...
class CalculateSatelliteAndSolarGrids(luigi.Task):
    ...
    def run(self):
        ...
        centreline_target = pjoin(out_path, CONFIG.get("work", "centreline_target"))
        header_angle_target = pjoin(out_path, CONFIG.get("work", "header_angle_target"))
        ...
        (
            ...
            acqs[0], lon_target, lat_target, npoints=12, out_fnames=targets
        )
...
class CalculateGridsTask(luigi.Task):
    ...
    def requires(self):
        return [
            CalculateLatLonGrids(self.l1t_path, self.out_path),
...
class CreateModisBrdfFiles(luigi.Task):
    ...
    def output(self):
        acqs = gaip.acquisitions(self.l1t_path)
        out_path = self.out_path
        modis_brdf_format = pjoin(out_path, CONFIG.get("brdf", "modis_brdf_format"))
...
class WriteTp5(luigi.Task):
    ...
    def run(self):
        ...
        sat_azi_fname = pjoin(out_path, CONFIG.get("work", "sat_azimuth_target"))
...
class CalculateCoefficients(luigi.Task):
    ...
    def requires(self):
        return [
            ExtractFlux(self.l1t_path, self.out_path),
...
class BilinearInterpolationBand(luigi.Task):
    ...
    def requires(self):
        return [
            ReformatAtmosphericParameters(self.l1t_path, self.out_path),
            CalculateSatelliteAndSolarGrids(self.l1t_path, self.out_path),
        ]
    ...
    def output(self):
        ...
        out_fname = output_format.format(factor=self.factor, band=self.band_num)
        return luigi.LocalTarget(out_fname)

    def run(self):
        out_path = self.out_path
        coordinator = pjoin(out_path, CONFIG.get("work", "coordinator_target"))
        boxline = pjoin(out_path, CONFIG.get("work", "boxline_target"))
        centreline = pjoin(out_path, CONFIG.get("work", "centreline_target"))
...
class BilinearInterpolation(luigi.Task):
            ...
            for band in bands:
                ...
                tasks.append(
                    BilinearInterpolationBand(
                        self.l1t_path, self.out_path, band, factor
                    )
                )
        return tasks
    ...
    def output(self):
        out_path = self.out_path
        target = pjoin(out_path, CONFIG.get("work", "bilinear_outputs_target"))
        return luigi.LocalTarget(target)
...
class DEMExctraction(luigi.Task):
    ...
    def output(self):
        ...
        subset_target = pjoin(work_path, CONFIG.get("extract_dsm", "dsm_subset"))
        smoothed_target = pjoin(
            work_path, CONFIG.get("extract_dsm", "dsm_smooth_subset")
        )
        targets = [luigi.LocalTarget(subset_target), luigi.LocalTarget(smoothed_target)]
        return targets
...
class SlopeAndAspect(luigi.Task):
    ...
    def output(self):
        ...
        header_slope_target = pjoin(
            work_path, CONFIG.get("work", "header_slope_target")
        )
...
class IncidentAngles(luigi.Task):
    ...
    def output(self):
        ...
        azi_incident_target = pjoin(
            work_path, CONFIG.get("self_shadow", "azimuth_incident_target")
        )
    ...
    def run(self):
        ...
        # Input targets
        solar_zenith_fname = pjoin(out_path, CONFIG.get("work", "solar_zenith_target"))
        solar_azimuth_fname = pjoin(
            out_path, CONFIG.get("work", "solar_azimuth_target")
        )
        slope_target = pjoin(work_path, CONFIG.get("self_shadow", "slope_target"))
        aspect_target = pjoin(work_path, CONFIG.get("self_shadow", "aspect_target"))
...
class ExitingAngles(luigi.Task):
    ...
    def output(self):
        ...
        azi_exiting_target = pjoin(
            work_path, CONFIG.get("self_shadow", "azimuth_exiting_target")
        )
    ...
    def run(self):
        ...
        # Input targets
        satellite_view_fname = pjoin(out_path, CONFIG.get("work", "sat_view_target"))
        satellite_azimuth_fname = pjoin(
            out_path, CONFIG.get("work", "sat_azimuth_target")
        )
        slope_target = pjoin(work_path, CONFIG.get("self_shadow", "slope_target"))
        aspect_target = pjoin(work_path, CONFIG.get("self_shadow", "aspect_target"))
...
class RelativeAzimuthSlope(luigi.Task):
    ...
    def requires(self):
        return [
            IncidentAngles(self.l1t_path, self.out_path),
    ...
    def output(self):
        ...
        relative_azimuth_slope_target = pjoin(
...
class SelfShadow(luigi.Task):
    ...
    def run(self):
        ...
        gaip.self_shadow(
            incident_target, exiting_target, self_shadow_target, x_tile, y_tile
        )
...
class CalculateCastShadow(luigi.Task):
    ...
    def requires(self):
        return [
            CalculateCastShadowSun(self.l1t_path, self.out_path),
...
class RunTCBand(luigi.Task):
    ...
    def requires(self):
        return [
            BilinearInterpolation(self.l1t_path, self.out_path),
            DEMExctraction(self.l1t_path, self.out_path),
    ...
    def output(self):
        ...
        for level in rfl_levels:
            target = pjoin(
                out_path, output_format.format(level=level, band=self.band_num)
            )
            targets.append(luigi.LocalTarget(target))
        return targets
    ...
    def run(self):
        ...
        bilinear_target = pjoin(out_path, CONFIG.get("work", "bilinear_outputs_target"))
        ...
        satellite_view_target = pjoin(out_path, CONFIG.get("work", "sat_view_target"))
        relative_angle_target = pjoin(
            out_path, CONFIG.get("work", "relative_azimuth_target")
        )
...
class WriteMetadata(luigi.Task):
    ...
    def run(self):
        ...
        targets = [
            pjoin(out_path, CONFIG.get("work", "aerosol_target")),
        ...
        sources = ["aerosol", "solar_distance", "water_vapour", "ozone", "elevation"]
