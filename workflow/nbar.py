...
class ReformatAsTp5Trans(luigi.Task):
    ...
    def output(self):
        ...
        workdir = pjoin(out_path, CONFIG.get("work", "reformat_tp5_trans_cwd"))
...
class SlopeAndSelfShadow(luigi.Task):
    ...
    def output(self):
        ...
        targets = [
            luigi.LocalTarget(self_shadow_target),
    ...
    def output(self):
        ...
        for level in rfl_levels:
            for band in bands_to_process:
                target = pjoin(out_path, output_format.format(level=level, band=band))
                targets.append(luigi.LocalTarget(target))
        return targets
...
if __name__ == "__main__":  # FIXME
    ...
    luigi.build([TerrainCorrection(l1t_path, out_path)], local_scheduler=True)
