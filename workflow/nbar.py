...
class CalculateSatelliteAndSolarGrids(luigi.Task):
    ...
    def output(self):
        ...
        fnames = {
            "sat_view": CONFIG.get("work", "sat_view_fname"),
        ...
        targets = {k: luigi.LocalTarget(pjoin(out_path, v)) for k, v in fnames.items()}
    ...
    def run(self):
        ...
        outputs = self.output()
        with outputs["sat_view"].temporary_path() as fname1, outputs[
            "sat_azimuth"
        ].temporary_path() as fname2, outputs[
            "sol_zenith"
        ].temporary_path() as fname3, outputs[
            "sol_azimuth"
        ].temporary_path() as fname4, outputs[
            "rel_azimuth"
        ].temporary_path() as fname5, outputs[
            "time"
        ].temporary_path() as fname6:
            out_fnames = [fname1, fname2, fname3, fname4, fname5, fname6]

            (_, _, _, _, _, _, y_cent, x_cent, n_cent) = gaip.calculate_angles(
...
class RunModtranCase(luigi.Task):
    ...
    def run(self):
        ...
        gaip.prepare_modtran(self.coord, self.albedo, modtran_work, modtran_exe)
...
class CalculateCoefficients(luigi.Task):
    ...
    def output(self):
        ...
        targets = {
            "coef1": luigi.LocalTarget(out_fname1),
    ...
    def run(self):
        ...
        coef1, coef2 = gaip.calculate_coefficients(
            coords, chn_input_format, dir_input_format, workpath
        )

        with self.output()["coef1"].temporary_path() as out_fname1, self.output()[
            "coef2"
        ].temporary_path() as out_fname2:
...
class BilinearInterpolationBand(luigi.Task):
    ...
    def requires(self):
        ...
        return {
            "coef": CalculateCoefficients(*args1),
    ...
    def output(self):
        ...
        out_fname = pjoin(workpath, output_format.format(band=band_num, factor=factor))
        return luigi.LocalTarget(out_fname)
    ...
    def run(self):
        ...
        with self.output().temporary_path() as out_fname:
            gaip.bilinear_interpolate(
                acq, coord_fname, box_fname, centre_fname, coefficients, out_fname
            )
...
class DEMExctraction(luigi.Task):
    ...
    def output(self):
        ...
        return {
            "subset": luigi.LocalTarget(pjoin(out_path, subset_fname)),
...
class SlopeAndAspect(luigi.Task):
    ...
    def output(self):
        ...
        aspect_fname = pjoin(out_path, CONFIG.get("self_shadow", "aspect_fname"))
        targets = {
            "slope": luigi.LocalTarget(slope_fname),
...
class IncidentAngles(luigi.Task):
    ...
    def output(self):
        ...
        targets = {
            "incident": luigi.LocalTarget(incident_fname),
...
class ExitingAngles(luigi.Task):
    ...
    def output(self):
        ...
        targets = {
            "exiting": luigi.LocalTarget(exiting_fname),
...
class CalculateCastShadowSun(luigi.Task):
    ...
    def run(self):
        ...
        # buffer = int(CONFIG.get('extract_dsm', 'dsm_buffer_width'))
        buffer = get_buffer(self.group)
        window_height = CONFIG.getint("terrain_correction", "shadow_sub_matrix_height")
        window_width = CONFIG.getint("terrain_correction", "shadow_sub_matrix_width")
...
class RunTCBand(luigi.Task):
    ...
    def output(self):
        ...
        targets = {
            "lambertian": luigi.LocalTarget(lambertian_fname),
