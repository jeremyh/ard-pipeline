...
class DEMExctraction(luigi.Task):
    ...
    def output(self):
        ...
        smoothed_target = CONFIG.get("terrain_correction", "dsm_smooth_subset")
    ...
    def run(self):
        ...
        subset_target = CONFIG.get("terrain_correction", "dsm_subset")
        smoothed_target = CONFIG.get("terrain_correction", "dsm_smooth_subset")
        CONFIG.get("terrain_correction", "dsm_buffer_width")
        dsm_subset_fname = pjoin(work_path, subset_target)
        dsm_subset_smooth_fname = pjoin(work_path, smoothed_target)

        gaip.get_dsm(acqs[0], national_dsm, dsm_subset_fname, dsm_subset_smooth_fname)
...
class TerrainCorrection(luigi.Task):
    ...
    def run(self):
        ...
        dsm_buffer_width = CONFIG.get("terrain_correction", "dsm_buffer_width")
        shadow_sub_matrix_height = CONFIG.get(
            "terrain_correction", "shadow_sub_matrix_height"
        )
        shadow_sub_matrix_width = CONFIG.get(
            "terrain_correction", "shadow_sub_matrix_width"
        )
        ...
        for level in rfl_levels:
            for band in bands_to_process:
                rfl_lvl_fnames[(band, level)] = output_format.format(
