...
class FractionalCoverTask(luigi.Task):
    ...
    def run(self):
        logging.info(
            "In FractionalCoverTask.run method, NBAR={}, "
            "output={}".format(self.input().nbar_path, self.output().path)
        )

        # create the output directory
        logging.debug(f"creating output directory {self.fc_path}")
        ...
        acqs = [
            ...
            for acq in acqs
            if (
                acq.band_type == gaip.REF
                and acq.wavelength[1] > min_lambda
                and acq.wavelength[1] <= max_lambda
            )
        ]
...
def main(nbar_path, outpath, nnodes=1, nodenum=1):
    nbar_files = sorted(
        [pjoin(nbar_path, f) for f in os.listdir(nbar_path) if "_NBAR_" in f]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_path",
        help=("path to directory where FC dataset is to " "be written"),
        required=True,
        type=lambda x: is_valid_directory(parser, x),
    )
    parser.add_argument(
    ...
    logfile = f"run_fc_{os.uname()[1]}_{os.getpid()}.log"
    logfile = os.path.join(args.log_path, logfile)
    logging_level = logging.INFO
    if args.debug:
        logging_level = logging.DEBUG
    logging.basicConfig(
        filename=logfile, level=logging_level, format=fmt, datefmt="%H:%M:%S"
    )
