...
L1T_PATTERN = (
    ...
    r"(?P<product_type>\w+)"
    r"_(?P<product_id>P\d+)_GA(?P<product_code>.*)-"
    r"(?P<station_id>\d+)_"
    r"(?P<wrs_path>\d+)_(?P<wrs_row>\d+)_"
...
class PixelQualityTask(luigi.Task):
    ...
    def run(self):
        ...
        logging.debug(
            ("l1t_data shape={}, " "geo_box= {}").format(
                bytes(l1t_data.shape), bytes(geo_box)
            )
        )
        ...
        logging.debug(
            ("nbar_data shape={}, " "geo_box= {}").format(
                bytes(nbar_data.shape), bytes(geo_box)
            )
        )
...
class PackagePQ(luigi.Task):
    ...
    def requires(self):
        return [
            PixelQualityTask(
                self.l1t_path, self.nbar_path, self.land_sea_path, self.pq_path
            )
        ]
    ...
    def run(self):
        ...
        with self.output().open("w") as src:
            src.write(f"{self.pq_path} packaging completed")
...
def main(l1t_path, nbar_path, land_sea_path, outpath, nnodes=1, nodenum=1):
    l1t_files = sorted(
        [pjoin(l1t_path, f) for f in os.listdir(l1t_path) if "_OTH_" in f]
    )
    files = zip(
        l1t_files,
        [pjoin(nbar_path, nbar_name_from_l1t(basename(f))) for f in l1t_files],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_path",
        help=("path to directory where PQA dataset is to be " "written"),
        required=True,
        type=lambda x: is_valid_directory(parser, x),
    )
    parser.add_argument(
    ...
    logfile = f"{args.log_path}/run_pq_{os.uname()[1]}_{os.getpid()}.log"
    ...
    logging.info(f"nbar_path={args.nbar_path}")
    logging.info(f"land_sea_path={args.land_sea_path}")
    ...
    main(args.l1t_path, args.nbar_path, args.land_sea_path, args.out_path, size, rank)
